...
import org.broadinstitute.dsde.test.api.tagannotation.rawls.{MethodsTestObj1, RawlsTestObj1, WorkspacesTestObj1}
...
class RawlsApiSpec
  ...
  "Rawls" - {
    ...
    "should be able to create workspace and run sub-workflow tasks in non-US regions" taggedAs(RawlsTestObj1) in {
      implicit val token: AuthToken = studentBToken
    ...
    "should be able to run sub-workflow tasks in a cloned workspace in non-US regions" taggedAs(RawlsTestObj1) in {
      implicit val token: AuthToken = studentBToken
...
//    Disabling this test until we decide what to do with it. See AP-177
    ...
    "should retrieve metadata with widely scattered sub-workflows in a short time" taggedAs(MethodsTestObj1) ignore {
    ...
    "should label low security bucket" taggedAs(WorkspacesTestObj1) in {
    ...
    "should label high security bucket" taggedAs(WorkspacesTestObj1) in {
      implicit val patienceConfig: PatienceConfig = PatienceConfig(timeout = 20 seconds)
      implicit val token: AuthToken = studentAToken
    ...
    "should have correct policies in Sam and IAM roles in Google when an unconstrained workspace is created" taggedAs(WorkspacesTestObj1) in {
      implicit val patienceConfig: PatienceConfig = PatienceConfig(timeout = 20 seconds)
    ...
    "should have correct policies in Sam and IAM roles in Google when a constrained workspace is created" taggedAs(WorkspacesTestObj1) in {
      implicit val patienceConfig: PatienceConfig = PatienceConfig(timeout = 20 seconds)
      implicit val token: AuthToken = ownerAuthToken
    ...
    "should clone a workspace and only copy files in the specified path" taggedAs(WorkspacesTestObj1) in {
    ...
    "should support running workflows with private docker images" taggedAs(MethodsTestObj1) in {
      implicit val token: AuthToken = ownerAuthToken
    ...
    "should support running workflows with wdl structs" taggedAs(MethodsTestObj1) in {
      implicit val token: AuthToken = ownerAuthToken
    ...
    "should fail to launch a submission with a reserved output attribute" taggedAs(MethodsTestObj1) in {
      implicit val token: AuthToken = ownerAuthToken
