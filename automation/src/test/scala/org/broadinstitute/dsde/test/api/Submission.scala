...
object Submission extends LazyLogging with Eventually with RandomUtil {
  ...
  def getSubmissionStatusAndWorkflowIds(billingProject: String, workspaceName: String, submissionId: String)(implicit
    token: AuthToken
  ): (String, List[String]) =
   ...
   */
  def waitUntilSubmissionComplete(billingProjectName: String, workspaceName: String, submissionId: String)(implicit
    token: AuthToken
  ): Unit = {
    implicit val patienceConfig: PatienceConfig =
      PatienceConfig(timeout = scaled(Span(20, Minutes)), interval = scaled(Span(30, Seconds)))

    eventually {
      val (actualStatus, workflowIds) =
        getSubmissionStatusAndWorkflowIds(billingProjectName, workspaceName, submissionId)
      withClue(
        s"Monitoring submission [$billingProjectName/$workspaceName/$submissionId] until finish; " +
