...
case class MarthaMinimalResponse(googleServiceAccount: Option[ServiceAccountPayload]) // Both Martha v2 and v3 return a JSON that includes `googleServiceAccount` as a top-level key
