...
object AvroUpsertMonitor {
  ...
  def props(pollInterval: FiniteDuration,
    ...
    Props(
      new AvroUpsertMonitorActor(pollInterval,
                                 pollIntervalJitter,
                                 entityService,
                                 googleServicesDAO,
                                 samDAO,
    ...
    try {
      ...
      def performUpsertBatch(idx: Long, upsertBatch: Seq[EntityUpdateDefinition]): Future[Traversable[Entity]] = {
        ...
        for {
          ...
          upsertResults <- entityService
            .apply(petUserInfo)
            .batchUpdateEntitiesInternal(workspace.toWorkspaceName, upsertBatch, upsert = isUpsert, None, None)
        } yield upsertResults
      }
