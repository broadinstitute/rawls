...
trait LocalEntityExpressionQueries {
  ...
  object entityExpressionQuery {
    ...
    def workspaceAttributeFinalQuery(attrName: AttributeName)(context: LocalEntityExpressionContext, shouldBeNone: Option[PipelineStepQuery]): ReadAction[Map[String, Iterable[Attribute]]] = {
    ...
    def entityNameAttributeRelationQuery(attrName: AttributeName)(context: LocalEntityExpressionContext, queryPipeline: PipelineStepQuery): PipelineStepQuery = {
      (for {
        ...
        attribute <- entityAttributeAllShardsViewQuery if entity.id === attribute.ownerId && attribute.name === attrName.name && attribute.namespace === attrName.namespace
        nextEntity <- entityQuery if attribute.valueEntityRef === nextEntity.id
    ...
    def entityAttributeFinalQuery(attrName: AttributeName)(context: LocalEntityExpressionContext, queryPipeline: Option[PipelineStepQuery]): ReadAction[Map[String, Iterable[Attribute]]] = {
      ...
      attributeForNameQuery.map { entityWithAttributeRecs =>
        ...
        byRootEnt.map { case (rootEnt, attrs) =>
          ...
          val attributesByEntityId: Map[String, AttributeMap] = entityAttributeAllShardsViewQuery.unmarshalAttributes(valueAttrRecs.map { case (root, attrEnt, attrRec) => ((attrEnt, attrRec), None) })
    ...
    def entityReservedAttributeFinalQuery(attributeName: String)(context: LocalEntityExpressionContext, queryPipeline: Option[PipelineStepQuery]): ReadAction[Map[String, Iterable[Attribute]]] = {
      ...
      def returnMapOfRootEntityToReservedAttribute(baseQuery: PipelineStepQuery, recordExtractionFn: EntityRecord => Attribute): ReadAction[Map[String, Iterable[Attribute]]] = {
    ...
    def workspaceReservedAttributeFinalQuery(attributeName: String)(context: LocalEntityExpressionContext, shouldBeNone: Option[PipelineStepQuery]): ReadAction[Map[String, Iterable[Attribute]]] = {
      assert(shouldBeNone.isEmpty)
    ...
    def entityFinalQuery(context: LocalEntityExpressionContext, queryPipeline: Option[PipelineStepQuery]): ReadAction[Map[String, Iterable[EntityRecord]]] = {
      queryPipeline.get.sortBy(_._2.name.asc).result.map(CollectionUtils.groupByTuples)
    }

    //Takes a list of entities at the end of a pipeline and returns them in final format.
    def workspaceEntityFinalQuery(attrName: AttributeName)(context: LocalEntityExpressionContext, shouldBeNone: Option[PipelineStepQuery]): ReadAction[Map[String, Iterable[EntityRecord]]] = {
      assert(shouldBeNone.isEmpty)
