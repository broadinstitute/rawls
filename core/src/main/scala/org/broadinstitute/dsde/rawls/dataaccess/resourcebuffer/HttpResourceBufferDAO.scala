...
class HttpResourceBufferDAO(config: ResourceBufferConfig, clientServiceAccountCreds: Credential)
  ...
  override def handoutGoogleProject(projectPoolId: ProjectPoolId, handoutRequestId: String): Future[GoogleProjectId] = {
    retry(when500) { () =>
      Future {
        ...
        val resource = handoutResourceGeneric(projectPoolId.value, handoutRequestId, OAuth2BearerToken(clientServiceAccountCreds.getAccessToken))
  ...
  private def handoutResourceGeneric(poolId: String, handoutRequestId: String, accessToken: OAuth2BearerToken): ResourceInfo =
    getResourceBufferApi(accessToken).handoutResource(new HandoutRequestBody().handoutRequestId(handoutRequestId), poolId)
