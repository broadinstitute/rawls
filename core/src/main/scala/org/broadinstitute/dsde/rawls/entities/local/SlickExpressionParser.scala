...
import org.broadinstitute.dsde.rawls.entities.local.SlickExpressionParser.PipeType
trait SlickExpressionParser extends ExpressionParser[ReadAction, SlickExpressionContext, PipeType] {
  ...
  protected override def entityRootFunc(context: SlickExpressionContext): PipeType = {
    for {
  ...
  protected override def workspaceAttributeFinalFunc(attrName: AttributeName)(context: SlickExpressionContext, shouldBeNone: Option[PipeType]): ReadAction[Map[String,Iterable[Attribute]]] = {
  ...
  protected override def workspaceEntityRefRootFunc(attrName: AttributeName)(context: SlickExpressionContext): PipeType = {
    for {
  ...
  protected override def entityNameAttributePipeFunc(attrName: AttributeName)(context: SlickExpressionContext, queryPipeline: PipeType): PipeType = {
    (for {
  ...
  protected override def entityAttributeFinalFunc(attrName: AttributeName)(context: SlickExpressionContext, queryPipeline: Option[PipeType]): ReadAction[Map[String, Iterable[Attribute]]] = {
  ...
  protected override def entityReservedAttributeFinalFunc(attributeName: String)(context: SlickExpressionContext, queryPipeline: Option[PipeType]): ReadAction[Map[String,Iterable[Attribute]]] = {
  ...
  protected override def workspaceReservedAttributeFinalFunc(attributeName: String)(context: SlickExpressionContext, shouldBeNone: Option[PipeType]): ReadAction[Map[String,Iterable[Attribute]]] = {
    assert(shouldBeNone.isEmpty)
  ...
  protected override def entityFinalFunc(context: SlickExpressionContext, queryPipeline: Option[PipeType]): ReadAction[Map[String,Iterable[EntityRecord]]] = {
    queryPipeline.get.sortBy(_._2.name.asc).result.map(CollectionUtils.groupByTuples)
  }

  //Takes a list of entities at the end of a pipeline and returns them in final format.
  protected override def workspaceEntityFinalFunc(attrName: AttributeName)(context: SlickExpressionContext, shouldBeNone: Option[PipeType]): ReadAction[Map[String,Iterable[EntityRecord]]] = {
    assert(shouldBeNone.isEmpty)
