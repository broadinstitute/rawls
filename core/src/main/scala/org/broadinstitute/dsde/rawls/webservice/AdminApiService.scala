...
trait AdminApiService extends UserInfoDirectives {
  ...
  val adminRoutes: server.Route = requireUserInfo() { userInfo =>
    ...
    path("admin" / "submissions") {
      get {
        complete {
          workspaceServiceConstructor(userInfo).adminListAllActiveSubmissions().map(StatusCodes.OK -> _)
        }
      }
    } ~
    ...
    path("admin" / "submissions" / "queueStatusByUser") {
      get {
        complete {
          workspaceServiceConstructor(userInfo).adminWorkflowQueueStatusByUser.map(StatusCodes.OK -> _)
        }
      }
    } ~
