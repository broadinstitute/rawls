  ...
  val adminRoutes: server.Route = requireUserInfo() { userInfo =>
    path("admin" / "billing" / Segment) { projectId =>
      ...
      path("admin" / "user" / "role" / "curator" / Segment) { userEmail =>
        put {
          complete {
            userServiceConstructor(userInfo).adminAddLibraryCurator(RawlsUserEmail(userEmail)).map(_ => StatusCodes.OK)
          }
        } ~
          delete {
            complete {
                ...
                case Some(attributeName) =>
                  ...
                  (stringOption, numberOption, booleanOption) match {
                    case (Some(string), None, None) =>
                      workspaceServiceConstructor(userInfo).adminListWorkspacesWithAttribute(name,
                                                                                             AttributeString(string)
                      )
                    case (None, Some(number), None) =>
                      workspaceServiceConstructor(userInfo).adminListWorkspacesWithAttribute(
                        name,
      ...
      path("admin" / "workspaces" / Segment / Segment / "flags") { (workspaceNamespace, workspaceName) =>
        get {
          complete {
            workspaceServiceConstructor(userInfo).adminListWorkspaceFeatureFlags(
          ...
          put {
            entity(as[List[String]]) { flagNames =>
              complete {
                workspaceServiceConstructor(userInfo).adminOverwriteWorkspaceFeatureFlags(
                  WorkspaceName(workspaceNamespace, workspaceName),
                  flagNames
                )
              }
