...
trait ExpressionValidator {
  ...
  )(implicit executionContext: ExecutionContext): ValidatedMethodConfiguration = {
    val validated = validateMCExpressions(methodConfiguration, gatherInputsResult)
    val errors = List(
      if (validated.invalidInputs.nonEmpty) Some(s"Invalid inputs: ${validated.invalidInputs.mkString(",")}") else None,
      if (validated.missingInputs.nonEmpty) Some(s"Missing inputs: ${validated.missingInputs.mkString(",")}") else None,
      if (validated.extraInputs.nonEmpty) Some(s"Extra inputs: ${validated.extraInputs.mkString(",")}") else None,
      if (validated.invalidOutputs.nonEmpty)
        Some(s"Invalid outputs: ${validated.invalidOutputs.mkString(",")}")
      else None
    ).flatten
    if (errors.nonEmpty) {
