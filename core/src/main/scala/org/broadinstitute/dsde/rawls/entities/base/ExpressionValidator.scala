...
import org.broadinstitute.dsde.rawls.model.{AttributeString, ErrorReport, MethodConfiguration, ParsedMCExpressions, ValidatedMethodConfiguration}
...
trait ExpressionValidator {
  ...
  protected def internalValidateMCExpressions(methodConfiguration: MethodConfiguration,
                                              gatherInputsResult: GatherInputsResult,
                                              allowRootEntity: Boolean,
                                              parseFunc: (Map[String, AttributeString], Map[String, AttributeString], Boolean) => ParsedMCExpressions): ValidatedMethodConfiguration = {
    ...
    val parsed = parseFunc(inputsToParse.toMap, outputsToParse, allowRootEntity)
