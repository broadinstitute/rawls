...
trait SamDAO extends ErrorReportable {
  ...
  @deprecated
  def overwriteManagedGroupMembership(groupName: WorkbenchGroupName, role: ManagedRoles.ManagedRole, memberEmails: List[WorkbenchEmail], userInfo: UserInfo): Future[Unit]
