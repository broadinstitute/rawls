...
class DataRepoEntityProvider(requestArguments: EntityRequestArguments, workspaceManagerDAO: WorkspaceManagerDAO, dataRepoDAO: DataRepoDAO)
  ...
  override def expressionValidator: ExpressionValidator = new DataRepoExpressionValidator(
    new DataRepoExpressionParser,
    dataRepoDAO.getSnapshot(lookupSnapshotForName(dataReferenceName), userInfo.accessToken)
  )
}
