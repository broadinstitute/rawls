...
object UserInfo {
  def buildFromTokens(credential: Credential): UserInfo = {
    ...
    UserInfo(RawlsUserEmail(""), token, Option(credential.getExpiresInSeconds).map(_.toLong).getOrElse(0), RawlsUserSubjectId(""))
  }

  def buildFromTokens(gCredential: GoogleCredentials): UserInfo = {
    val token = gCredential.refreshAccessToken
    UserInfo(RawlsUserEmail(""), OAuth2BearerToken(token.getTokenValue), Option(token.getExpirationTime).map(_.getTime).getOrElse(0), RawlsUserSubjectId(""))
  }
}
