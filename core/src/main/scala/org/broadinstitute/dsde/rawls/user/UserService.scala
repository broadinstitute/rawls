...
class UserService(protected val ctx: RawlsRequestContext,
  ...
  protected val userInfo: UserInfo = ctx.userInfo
  ...
  def listBillingAccounts(): Future[Seq[RawlsBillingAccount]] =
    gcsDAO.listBillingAccounts(ctx.userInfo)
  ...
  def getBillingProject(billingProjectName: RawlsBillingProjectName): Future[Option[RawlsBillingProjectResponse]] = {
    for {
      projectRoles <- samDAO.listUserRolesForResource(SamResourceTypeNames.billingProject, billingProjectName.value, ctx.userInfo)
        .map(resourceRoles => samRolesToProjectRoles(resourceRoles.map(_.value)))
  ...
  def listBillingProjectsV2(): Future[List[RawlsBillingProjectResponse]] = {
    for {
      ...
      projectNames = samUserResources.map(r => RawlsBillingProjectName(r.getResourceId)).toSet
      projectsInDB <- dataSource.inTransaction { dataAccess =>
        dataAccess.rawlsBillingProjectQuery.getBillingProjects(projectNames)
      }
      bpmProfiles <- billingProfileManagerDAO.listBillingProfiles(ctx.userInfo, samUserResources)
    } yield constructBillingProjectResponses(samUserResources, projectsInDB ++ bpmProfiles)
  }

  private def constructBillingProjectResponses(samUserResources: Seq[UserResourcesResponse], billingProjectsInRawlsDB: Seq[RawlsBillingProject]): List[RawlsBillingProjectResponse] = {
    ...
    samUserResources.toList
      .flatMap { samUserResource =>
        val projectRoles = samRolesToProjectRoles(samUserResource.getDirect.getRoles.asScala.toSet ++ samUserResource.getInherited.getRoles.asScala.toSet)
        projectsByName.get(samUserResource.getResourceId).map(makeBillingProjectResponse(projectRoles, _))
      }
  ...
  private def samRolesToProjectRoles(samRoles: Set[String]): Set[ProjectRole] = {
    samRoles.collect {
  ...
  def createBillingProjectV2(createProjectRequest: CreateRawlsV2BillingProjectFullRequest): Future[Unit] = {
    for {
      ...
      _ <- checkServicePerimeterAccess(createProjectRequest.servicePerimeter)
      hasAccess <- gcsDAO.testBillingAccountAccess(createProjectRequest.billingAccount, ctx.userInfo)
      _ = if (!hasAccess) {
        throw new RawlsExceptionWithErrorReport(ErrorReport(StatusCodes.BadRequest, "Billing account does not exist, user does not have access, or Terra does not have access"))
      }
  ...
  private def createV2BillingProjectInternal(createProjectRequest: CreateRawlsV2BillingProjectFullRequest): Future[Unit] = {
    for {
      ...
      _ <- samDAO.createResourceFull(SamResourceTypeNames.billingProject, createProjectRequest.projectName.value, defaultBillingProjectPolicies, Set.empty, ctx.userInfo, None)
