...
object UserService {
  ...
  private def deletePetsInProject(projectName: GoogleProjectId,
    ...
    for {
      ...
      _ <- projectUsers.toList.traverse(destroyPet(_, projectName, gcsDAO, samDAO))
    } yield ()
...
class UserService(
  ...
  def listBillingProjectsV2(): Future[List[RawlsBillingProjectResponse]] = for {
    ...
    hardcodedBillingProject <- billingProfileManagerDAO.getHardcodedAzureBillingProject(resourceIds, ctx)
