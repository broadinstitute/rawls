...
object UserService {
  ...
  case class AdminUnregisterBillingProject(projectName: RawlsBillingProjectName, xfer: RawlsBillingProjectTransfer) extends UserServiceMessage
...
class UserService(protected val userInfo: UserInfo, val dataSource: SlickDataSource, protected val gcsDAO: GoogleServicesDAO, gpsDAO: GooglePubSubDAO, gpsGroupSyncTopic: String, notificationDAO: NotificationDAO, samDAO: SamDAO, projectOwnerGrantableRoles: Seq[String])(implicit protected val executionContext: ExecutionContext) extends Actor with RoleSupport with FutureSupport with UserWiths with LazyLogging with DirectorySubjectNameSupport {
  ...
  override def receive = {
    ...
    case AdminUnregisterBillingProject(projectName, xfer) => asFCAdmin { unregisterBillingProject(projectName, xfer) } pipeTo sender
  ...
  def unregisterBillingProject(projectName: RawlsBillingProjectName, xfer: RawlsBillingProjectTransfer): Future[PerRequestMessage] = {
    val ownerUserInfo = UserInfo(RawlsUserEmail(xfer.ownerEmail), OAuth2BearerToken(xfer.ownerToken), 3600, RawlsUserSubjectId("0"))
    for {
