...
trait EntityTypeStatisticsComponent {
  ...
  class EntityTypeStatisticsTable(tag: Tag) extends Table[EntityTypeStatisticsRecord](tag, "WORKSPACE_ENTITY_TYPE_STATISTICS") {
    def workspaceId = column[UUID]("WORKSPACE_ID")
    def entityType = column[String]("ENTITY_TYPE", O.Length(254))
    def entityCount = column[Long]("ENTITY_TYPE_COUNT")

    def * = (workspaceId, entityType, entityCount) <> (EntityTypeStatisticsRecord.tupled, EntityTypeStatisticsRecord.unapply)
  }
  ...
  object entityTypeStatisticsQuery extends TableQuery(new EntityTypeStatisticsTable(_)) {
    ...
    def getAll(workspaceId: UUID): ReadAction[Map[String, Int]] = {
      entityTypeStatisticsQuery.filter(_.workspaceId === workspaceId).map { res =>
