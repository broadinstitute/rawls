...
/**
 * Utility for matching a path to a Swagger route and extracting parameters.
 * Routes contain parameter names in curly braces, e.g. /api/workspaces/{workspaceNamespace}/{workspaceName}.
 * Paths are the routes with the parameters filled in e.g. /api/workspaces/a_real_namespace/a_real_workspace_name.
 * This class uses the Swagger API documentation to match paths to routes and extract parameters.
 */
object SwaggerRouteMatcher {
   ...
   * @return the matched route and extracted parameters, if any
   */
