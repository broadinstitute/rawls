...
object SnapshotService {
  ...
  )(userInfo: UserInfo)(implicit executionContext: ExecutionContext): SnapshotService =
...
class SnapshotService(protected val userInfo: UserInfo,
  ...
  def getSnapshot(workspaceName: WorkspaceName, referenceId: String): Future[DataRepoSnapshotResource] = {
    ...
    getWorkspaceContextAndPermissions(workspaceName,
      ...
      val ref = workspaceManagerDAO.getDataRepoSnapshotReference(workspaceContext.workspaceIdAsUUID,
                                                                 referenceUuid,
                                                                 userInfo.accessToken
      )
  ...
  protected[snapshot] def retrieveSnapshotReferences(workspaceId: UUID, offset: Int, limit: Int): SnapshotListResponse =
    Try(
      workspaceManagerDAO.enumerateDataRepoSnapshotReferences(workspaceId, offset, limit, userInfo.accessToken)
    ) match {
  ...
  def updateSnapshot(workspaceName: WorkspaceName,
    ...
    getWorkspaceContextAndPermissions(workspaceName,
      ...
      workspaceManagerDAO.getDataRepoSnapshotReference(workspaceContext.workspaceIdAsUUID,
                                                       snapshotUuid,
                                                       userInfo.accessToken
      )
  ...
  def deleteSnapshot(workspaceName: WorkspaceName, snapshotId: String): Future[Unit] = {
    ...
    getWorkspaceContextAndPermissions(workspaceName,
      ...
      workspaceManagerDAO.deleteDataRepoSnapshotReference(workspaceContext.workspaceIdAsUUID,
                                                          snapshotUuid,
                                                          userInfo.accessToken
      )
    }
