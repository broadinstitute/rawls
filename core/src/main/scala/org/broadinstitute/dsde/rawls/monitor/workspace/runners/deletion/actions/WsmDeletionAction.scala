...
class WsmDeletionAction(workspaceManagerDao: WorkspaceManagerDAO)(implicit val system: ActorSystem)
  ...
  private def isComplete(workspace: Workspace, jobId: String, ctx: RawlsRequestContext)(implicit
    ...
    case Failure(e: ApiException)
      ...
      logger.info(
        s"Workspace deletion result status = ${e.getCode}, WSM record is gone. Proceeding with rawls workspace deletion [workspaceId=${workspace.workspaceId}, jobControlId=${jobId}]"
      )
      true
  ...
  private def startDeletion(workspace: Workspace, jobControlId: String, ctx: RawlsRequestContext)(implicit
    ...
    Future {
      blocking {
        logger.info(
          s"Starting WSM workspace deletion [workspaceId=${workspace.workspaceId}, jobControlId=${jobControlId}]"
        )
