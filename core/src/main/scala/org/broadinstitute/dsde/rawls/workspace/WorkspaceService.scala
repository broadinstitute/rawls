...
object WorkspaceService {
  ...
  )(userInfo: UserInfo)(implicit materializer: Materializer, executionContext: ExecutionContext): WorkspaceService =
    new WorkspaceService(userInfo,
                         dataSource,
                         entityManager,
                         methodRepoDAO,
                         cromiamDAO,
...
class WorkspaceService(protected val userInfo: UserInfo,
  ...
  def createWorkspace(workspaceRequest: WorkspaceRequest, parentSpan: Span = null): Future[Workspace] =
    traceWithParent("withAttributeNamespaceCheck", parentSpan)(s1 =>
  ...
  def getWorkspace(workspaceName: WorkspaceName,
    ...
    traceWithParent("getWorkspaceContextAndPermissions", parentSpan)(s1 =>
              ...
              def workspaceAuthorizationDomainFuture(): Future[Option[Set[ManagedGroupRef]]] =
                if (options.contains("workspace.authorizationDomain") || options.contains("workspace")) {
                  traceWithParent("loadResourceAuthDomain", s1)(_ =>
                    loadResourceAuthDomain(SamResourceTypeNames.workspace, workspaceContext.workspaceId, userInfo)
                      .map(Option(_))
                  )
                } else {
                  noFuture
  ...
  def getWorkspaceOwners(workspaceId: String): Future[Set[WorkbenchEmail]] =
    samDAO
      .getPolicy(SamResourceTypeNames.workspace, workspaceId, SamWorkspacePolicyNames.owner, userInfo)
      .map(_.memberEmails)

  def deleteWorkspace(workspaceName: WorkspaceName, parentSpan: Span = null): Future[Option[String]] =
    traceWithParent("getWorkspaceContextAndPermissions", parentSpan)(_ =>
      getWorkspaceContextAndPermissions(workspaceName, SamWorkspaceActions.delete) flatMap { ctx =>
        traceWithParent("maybeLoadMCWorkspace", parentSpan)(_ => maybeLoadMcWorkspace(ctx)) flatMap {
          maybeMcWorkspace =>
            traceWithParent("deleteWorkspaceInternal", parentSpan)(s1 =>
              deleteWorkspaceInternal(workspaceName, ctx, maybeMcWorkspace, s1)
            )
        }
      }
    )
  ...
  def maybeLoadMcWorkspace(workspaceContext: Workspace): Future[Option[WorkspaceDescription]] =
    workspaceContext.workspaceType match {
      case WorkspaceType.McWorkspace =>
        Future(Option(workspaceManagerDAO.getWorkspace(workspaceContext.workspaceIdAsUUID, userInfo.accessToken)))
  ...
  private def deleteWorkspaceInternal(workspaceName: WorkspaceName,
    ...
    for {
      _ <- traceWithParent("requesterPaysSetupService.revokeAllUsersFromWorkspace", parentSpan)(_ =>
        requesterPaysSetupService.revokeAllUsersFromWorkspace(workspaceContext) recoverWith { case t: Throwable =>
          logger.warn(
            s"Unexpected failure deleting workspace (while revoking 'requester pays' users) for workspace `${workspaceName}`",
            t
          )
          Future.failed(t)
        }
      ...
      workflowsToAbort <- traceWithParent("gatherWorkflowsToAbortAndSetStatusToAborted", parentSpan)(_ =>
        gatherWorkflowsToAbortAndSetStatusToAborted(workspaceName, workspaceContext) recoverWith { case t: Throwable =>
          logger.warn(
            s"Unexpected failure deleting workspace (while gathering workflows that need to be aborted) for workspace `${workspaceName}`",
            t
          )
          Future.failed(t)
        }
      ...
      aborts = traceWithParent("abortRunningWorkflows", parentSpan)(_ =>
        Future.traverse(workflowsToAbort)(wf => executionServiceCluster.abort(wf, userInfo)) recoverWith {
          case t: Throwable =>
            logger.warn(
              s"Unexpected failure deleting workspace (while aborting workflows) for workspace `${workspaceName}`",
              t
            )
            Future.failed(t)
        }
      ...
      _ <- traceWithParent("maybeDeleteGoogleProject", parentSpan)(_ =>
        if (!isAzureMcWorkspace(maybeMcWorkspace)) {
          maybeDeleteGoogleProject(workspaceContext.googleProjectId,
            ...
            logger.error(
              s"Unexpected failure deleting workspace (while deleting google project) for workspace `${workspaceName}`",
              t
            )
            Future.failed(t)
          }
      ...
      _ <- traceWithParent("deleteWorkspaceTransaction", parentSpan)(_ =>
        deleteWorkspaceTransaction(workspaceName, workspaceContext) recoverWith { case t: Throwable =>
          logger.error(
            s"Unexpected failure deleting workspace (while deleting workspace in Rawls DB) for workspace `${workspaceName}`",
            t
          )
          Future.failed(t)
        }
      ...
      _ <- traceWithParent("deleteWorkflowCollectionSamResource", parentSpan)(_ =>
        workspaceContext.workflowCollectionName
          .map(cn => samDAO.deleteResource(SamResourceTypeNames.workflowCollection, cn, userInfo))
          ...
          case t: RawlsExceptionWithErrorReport if t.errorReport.statusCode.contains(StatusCodes.NotFound) =>
            logger.warn(
              s"Received 404 from delete workflowCollection resource in Sam (while deleting workspace) for workspace `${workspaceName}`: [${t.errorReport.message}]"
            )
            Future.successful()
          case t: RawlsExceptionWithErrorReport =>
            logger.error(
              s"Unexpected failure deleting workspace (while deleting workflowCollection in Sam) for workspace `${workspaceName}`.",
              t
            )
            Future.failed(t)
        }
      ...
      _ <- traceWithParent("deleteWorkspaceInWSM", parentSpan)(_ =>
        Future(workspaceManagerDAO.deleteWorkspace(workspaceContext.workspaceIdAsUUID, userInfo.accessToken))
          .recoverWith {
            // this will only ever succeed if a TDR snapshot had been created in the WS, so we gracefully handle all exceptions here
            case e: ApiException =>
              if (e.getCode != StatusCodes.NotFound.intValue) {
                logger.warn(
                  s"Unexpected failure deleting workspace (while deleting in Workspace Manager) for workspace `${workspaceName}. Received ${e.getCode}: [${e.getResponseBody}]"
                )
              }
              Future.successful()
          }
      ...
      _ <- traceWithParent("deleteWorkspaceSamResource", parentSpan)(_ =>
        if (workspaceContext.workspaceType != WorkspaceType.McWorkspace) { // WSM will delete Sam resources for McWorkspaces
          samDAO.deleteResource(SamResourceTypeNames.workspace,
            ...
            case t: RawlsExceptionWithErrorReport if t.errorReport.statusCode.contains(StatusCodes.NotFound) =>
              logger.warn(
                s"Received 404 from delete workspace resource in Sam (while deleting workspace) for workspace `${workspaceName}`: [${t.errorReport.message}]"
              )
              Future.successful()
            case t: RawlsExceptionWithErrorReport =>
              logger.error(
                s"Unexpected failure deleting workspace (while deleting workspace in Sam) for workspace `${workspaceName}`.",
                t
              )
              Future.failed(t)
          }
  ...
  def listWorkspaces(params: WorkspaceFieldSpecs, parentSpan: Span): Future[JsValue] = {
    ...
    for {
      workspacePolicies <- traceWithParent("getPolicies", parentSpan)(_ =>
        samDAO.getPoliciesForType(SamResourceTypeNames.workspace, userInfo)
      )
      ...
      result <- dataSource.inTransaction(
        { dataAccess =>
          ...
          val query: ReadAction[(Map[UUID, WorkspaceSubmissionStats], Seq[Workspace])] = for {
            submissionSummaryStats <- traceReadOnlyDBIOWithParent("submissionStats", parentSpan)(_ =>
              workspaceSubmissionStatsFuture()
            )
            workspaces <- traceReadOnlyDBIOWithParent("listByIds", parentSpan)(_ =>
          ...
          val results = traceDBIOWithParent("finalResults", parentSpan)(_ =>
  ...
  def getCatalog(workspaceName: WorkspaceName): Future[Set[WorkspaceCatalog]] =
    loadWorkspaceId(workspaceName).flatMap { workspaceId =>
      samDAO.getPolicy(SamResourceTypeNames.workspace, workspaceId, SamWorkspacePolicyNames.canCatalog, userInfo).map {
        members => members.memberEmails.map(email => WorkspaceCatalog(email.value, true))
      }
    }
            ...
            inviteNotifications <- Future.traverse(userToInvite) { invite =>
              samDAO.inviteUser(invite, userInfo).map { _ =>
                Notifications.WorkspaceInvitedNotification(
                  ...
                  WorkbenchUserId(userInfo.userSubjectId.value),
  ...
  private def gatherMethodConfigInputs(
    ...
    toFutureTry(methodRepoDAO.getMethod(methodConfig.methodRepoMethod, userInfo)).map {
      ...
      case Success(Some(wdl)) =>
        methodConfigResolver
          .gatherInputs(userInfo, methodConfig, wdl)
    ...
    asFCAdmin {
      for {
        ...
        results <- Future.traverse(workspaces) { workspace =>
          loadResourceAuthDomain(SamResourceTypeNames.workspace, workspace.workspaceId, userInfo).map(
  ...
  def getGenomicsOperationV2(workflowId: String, operationId: List[String]): Future[Option[JsObject]] =
    ...
    cromiamDAO.callLevelMetadata(workflowId, MetadataParams(includeKeys = Set("jobId")), userInfo).flatMap {
      metadataJson =>
  ...
  private def createWorkflowCollectionForWorkspace(workspaceId: String,
    ...
    for {
      _ <- traceWithParent("createResourceFull", parentSpan)(_ =>
  ...
  def updateAndGetBillingAccountAccess(billingProject: RawlsBillingProject,
    ...
    for {
      hasAccess <- traceWithParent("checkBillingAccountIAM", parentSpan)(_ =>
  ...
  private def withNewWorkspaceContext[T](workspaceRequest: WorkspaceRequest,
    ...
    traceDBIOWithParent("requireCreateWorkspaceAccess", parentSpan)(span =>
      requireCreateWorkspaceAccess(workspaceRequest, dataAccess, span) {
        traceDBIOWithParent("maybeRequireBillingProjectOwnerAccess", parentSpan)(_ =>
          maybeRequireBillingProjectOwnerAccess(workspaceRequest) {
            traceDBIOWithParent("findByName", parentSpan)(_ =>
              ...
              case None =>
                ...
                for {
                  billingProjectOwnerPolicyEmail <- traceDBIOWithParent("getPolicySyncStatus", parentSpan)(_ =>
                    DBIO.from(
                      samDAO
                  ...
                  _ <- DBIO.from {
                    ...
                    val createWorkflowCollectionFuture = traceWithParent("createWorkflowCollectionForWorkspace",
                                                                         parentSpan
                    )(span => createWorkflowCollectionForWorkspace(workspaceId, policyEmailsByName, span))
                  ...
                  (googleProjectId, googleProjectNumber) <- traceDBIOWithParent("setupGoogleProject", parentSpan) {
                    span =>
                      DBIO
                        .from(
                          for {
                  ...
                  _ <- traceDBIOWithParent("createResourceFull (google project)", parentSpan)(_ =>
                    DBIO.from(
                      samDAO.createResourceFull(
                  ...
                  workspaceBucketLocation <- traceDBIOWithParent("determineWorkspaceBucketLocation", parentSpan)(_ =>
                    DBIO.from(
                  ...
                  _ <- traceDBIOWithParent("gcsDAO.setupWorkspace", parentSpan)(span =>
                    DBIO.from(
