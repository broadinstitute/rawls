...
object Boot extends IOApp with LazyLogging {
  ...
  def initAppDependencies[F[_]: Logger: Async](config: Config, appName: String, metricsPrefix: String)(implicit
    ...
    val storageConfig = config.getConfig("gcs")
    val serviceProject = GoogleProject(storageConfig.getString("serviceProject"))
    val oidcConfig = config.getConfig("oidc")

    // todo: load these credentials once [CA-1806]
    val pathToCredentialJson = storageConfig.getString("pathToCredentialJson")
    ...
    val googleApiUri = Uri.unsafeFromString(storageConfig.getString("google-api-uri"))
