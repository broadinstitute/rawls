...
object InputExpressionReassembler {
   ...
   */
  private def groupByEntityName(seqOfEntityToLookupExprAndValue: Seq[(EntityName, (LookupExpression, Try[JsValue]))])
   ...
   */
  private def convertEvaluatedExprSeqToMap(mapOfEntityToSeqOfEvaluatedExpr: Map[EntityName, Seq[(LookupExpression, Try[JsValue])]])
