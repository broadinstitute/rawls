...
class SubmissionCostService(implicit val executionContext: ExecutionContext) {


  def getWorkflowCosts(workflowIds: Seq[String], userInfo: UserInfo, submission: Submission): Future[Map[String, Float]] = {
...
//
//    val subqueryTemplate = submission.workflows.map(id => s"labels_value LIKE $id")
//
//
//
//    val queryString: String = "SELECT COUNT(*) AS scullion_count FROM publicdata.samples.shakespeare WHERE word='scullion'"
//    val googleProject: GoogleProject = ??? // I think rawls?
//
