...
trait RawlsApiService //(val workspaceServiceConstructor: RawlsRequestContext => WorkspaceService, val userServiceConstructor: RawlsRequestContext => UserService, val genomicsServiceConstructor: RawlsRequestContext => GenomicsService, val statusServiceConstructor: () => StatusService, val executionServiceCluster: ExecutionServiceCluster, val appVersion: ApplicationVersion, val googleClientId: String, val submissionTimeout: FiniteDuration, override val workbenchMetricBaseName: String, val samDAO: SamDAO, val swaggerConfig: SwaggerConfig)(implicit val executionContext: ExecutionContext, val materializer: Materializer)
