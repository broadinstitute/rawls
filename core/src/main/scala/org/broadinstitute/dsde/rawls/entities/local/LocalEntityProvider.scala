 ...
 */
class LocalEntityProvider(workspace: Workspace, protected val dataSource: SlickDataSource)
  ...
  override def evaluateExpressions(workspaceContext: SlickWorkspaceContext, expressionEvaluationContext: ExpressionEvaluationContext, gatherInputsResult: GatherInputsResult): Future[Stream[SubmissionValidationEntityInputs]] = {
    dataSource.inTransaction { dataAccess =>
      withEntityRecsForExpressionEval(expressionEvaluationContext, workspaceContext, dataAccess) { jobEntityRecs =>
        ...
        evaluateInputExpressions(workspaceContext, gatherInputsResult.processableInputs, jobEntityRecs, dataAccess) map { valuesByEntity =>
          valuesByEntity
  ...
  private def withEntityRecsForExpressionEval[T](expressionEvaluationContext: ExpressionEvaluationContext, workspaceContext: SlickWorkspaceContext, dataAccess: DataAccess)(op: (Option[Seq[EntityRecord]]) => ReadWriteAction[T]): ReadWriteAction[T] = {
