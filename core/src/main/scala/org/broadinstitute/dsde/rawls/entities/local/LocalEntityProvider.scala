 ...
 */
class LocalEntityProvider(workspace: Workspace,
  ...
  override def entityTypeMetadata(useCache: Boolean): Future[Map[String, EntityTypeMetadata]] =
    ...
    trace("LocalEntityProvider.entityTypeMetadata") { rootSpan =>
      rootSpan.putAttribute("workspace",
                            OpenCensusAttributeValue.stringAttributeValue(workspace.toWorkspaceName.toString)
      )
            ...
            case Some(stalenessSeconds) =>
              ...
              cacheFeatureFlags(dataAccess, rootSpan).flatMap { flags =>
                if (flags.alwaysCacheTypeCounts || flags.alwaysCacheAttributes) {
                  rootSpan.putAttribute("alwaysCacheTypeCountsFeatureFlag",
                                        OpenCensusAttributeValue.booleanAttributeValue(flags.alwaysCacheTypeCounts)
                  )
                  rootSpan.putAttribute("alwaysCacheAttributesFeatureFlag",
            ...
            new RawlsExceptionWithErrorReport(
              errorReport =
                ErrorReport(StatusCodes.Conflict,
                            s"${entity.entityType} ${entity.name} already exists in ${workspace.toWorkspaceName}"
                )
            )
          )
  ...
  override def deleteEntities(entRefs: Seq[AttributeEntityReference]): Future[Int] =
    dataSource.inTransaction { dataAccess =>
      ...
      traceDBIO("LocalEntityProvider.deleteEntities") { rootSpan =>
        rootSpan.putAttribute("workspaceId",
        ...
        withAllEntityRefs(workspaceContext, dataAccess, entRefs, rootSpan) { _ =>
          traceDBIOWithParent("entityQuery.getAllReferringEntities", rootSpan)(innerSpan =>
  ...
  override def evaluateExpressions(expressionEvaluationContext: ExpressionEvaluationContext,
    ...
    dataSource.inTransaction { dataAccess =>
      withEntityRecsForExpressionEval(expressionEvaluationContext, workspace, dataAccess) { jobEntityRecs =>
        ...
        evaluateExpressionsInternal(workspace, gatherInputsResult.processableInputs, jobEntityRecs, dataAccess) map {
          valuesByEntity =>
  ...
  def batchUpdateEntitiesImpl(entityUpdates: Seq[EntityUpdateDefinition],
    ...
    trace("LocalEntityProvider.batchUpdateEntitiesImpl") { rootSpan =>
      ...
      rootSpan.putAttribute("entityOperationsCount",
      ...
      withAttributeNamespaceCheck(namesToCheck) {
        dataSource.inTransactionWithAttrTempTable(Set(AttributeTempTableType.Entity)) { dataAccess =>
          val updateTrialsAction = traceDBIOWithParent("getActiveEntities", rootSpan)(_ =>
