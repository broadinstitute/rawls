...
trait WorkspaceSupport {
  ...
  def accessCheck(workspace: Workspace, requiredAction: SamResourceAction, ignoreLock: Boolean): Future[Unit] =
    samDAO.userHasAction(SamResourceTypeNames.workspace, workspace.workspaceId, requiredAction, userInfo) flatMap {
      hasRequiredLevel =>
        if (hasRequiredLevel) {
  ...
  def requireCreateWorkspaceAccess[T](workspaceRequest: WorkspaceRequest,
    ...
    for {
      userHasAction <- traceDBIOWithParent("userHasAction", parentSpan)(_ =>
        DBIO.from(
  ...
  def maybeRequireBillingProjectOwnerAccess[T](workspaceRequest: WorkspaceRequest, parentSpan: Span = null)(
    op: => ReadWriteAction[T]
  ): ReadWriteAction[T] =
    workspaceRequest.noWorkspaceOwner match {
      case Some(true) =>
        for {
          billingProjectRoles <- traceDBIOWithParent("listUserRolesForResource", parentSpan)(_ =>
            DBIO.from(
              samDAO.listUserRolesForResource(SamResourceTypeNames.billingProject, workspaceRequest.namespace, userInfo)
            )
          )
