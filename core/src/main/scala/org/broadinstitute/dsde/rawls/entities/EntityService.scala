...
object EntityService {
  def constructor(dataSource: SlickDataSource,
                  ...
                  pageSizeLimit: Int
  )(userInfo: UserInfo)(implicit executionContext: ExecutionContext): EntityService =
    new EntityService(userInfo, dataSource, samDAO, entityManager, workbenchMetricBaseName, pageSizeLimit)
}

class EntityService(protected val userInfo: UserInfo,
    ...
    getWorkspaceContextAndPermissions(entityCopyDef.destinationWorkspace,
      ...
      getWorkspaceContextAndPermissions(entityCopyDef.sourceWorkspace,
        ...
        dataSource.inTransaction { dataAccess =>
          for {
            sourceAD <- DBIO.from(
              samDAO.getResourceAuthDomain(SamResourceTypeNames.workspace, sourceWorkspaceContext.workspaceId, userInfo)
            )
            destAD <- DBIO.from(
              samDAO.getResourceAuthDomain(SamResourceTypeNames.workspace, destWorkspaceContext.workspaceId, userInfo)
            )
            result <- authDomainCheck(sourceAD.toSet, destAD.toSet) flatMap { _ =>
              val entityNames = entityCopyDef.entityNames
              val entityType = entityCopyDef.entityType
              val copyResults = traceDBIOWithParent("checkAndCopyEntities", parentSpan)(s1 =>
