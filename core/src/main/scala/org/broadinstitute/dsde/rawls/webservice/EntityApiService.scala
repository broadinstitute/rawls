      ...
      path("workspaces" / Segment / Segment / "entityQuery" / Segment) {
                ...
                traceRequest { span =>
                  parameterSeq { allParams =>
                    ...
                    if (errors.isEmpty) {
                      ...
                      complete {
                        entityServiceConstructor(userInfo).queryEntities(WorkspaceName(workspaceNamespace,
                                                                                       workspaceName
                                                                         ),
                                                                         dataReference,
                                                                         entityType,
        ...
        path("workspaces" / Segment / Segment / "entities") { (workspaceNamespace, workspaceName) =>
          get {
            ...
            parameters('useCache.?) { useCache =>
              ...
              complete {
                entityServiceConstructor(userInfo).entityTypeMetadata(WorkspaceName(workspaceNamespace, workspaceName),
                                                                      dataReference,
                                                                      None,
                                                                      useCacheBool
                )
          ...
          post {
            entity(as[Entity]) { entity =>
              addLocationHeader(entity.path(WorkspaceName(workspaceNamespace, workspaceName))) {
                complete {
                  entityServiceConstructor(userInfo)
        ...
        path("workspaces" / Segment / Segment / "entities" / Segment / Segment) {
          (workspaceNamespace, workspaceName, entityType, entityName) =>
            get {
              complete {
                entityServiceConstructor(userInfo).getEntity(WorkspaceName(workspaceNamespace, workspaceName),
                                                             entityType,
                                                             entityName,
                                                             dataReference,
                                                             billingProject
          ...
          (workspaceNamespace, workspaceName, entityType, entityName) =>
            patch {
              entity(as[Array[AttributeUpdateOperation]]) { operations =>
                complete {
                  entityServiceConstructor(userInfo).updateEntity(WorkspaceName(workspaceNamespace, workspaceName),
                                                                  entityType,
                                                                  entityName,
                                                                  operations
                  )
          ...
          (workspaceNamespace, workspaceName, entityType, entityName) =>
            post {
              entity(as[String]) { expression =>
                complete {
                  entityServiceConstructor(userInfo).evaluateExpression(WorkspaceName(workspaceNamespace,
                                                                                      workspaceName
                                                                        ),
                                                                        entityType,
                                                                        entityName,
        ...
        path("workspaces" / Segment / Segment / "entities" / Segment) {
          (workspaceNamespace, workspaceName, entityType) =>
            get {
              traceRequest { span =>
                complete {
                  entityServiceConstructor(userInfo).listEntities(WorkspaceName(workspaceNamespace, workspaceName),
                                                                  entityType,
                                                                  span
                  )
                }
              ...
              extractRequest { request =>
                ...
                entity(as[EntityCopyDefinition]) { copyDefinition =>
                  traceRequest { span =>
                    complete {
                      entityServiceConstructor(userInfo)
                        .copyEntities(copyDefinition, request.uri, linkExistingEntitiesBool, span)
                        .map { response =>
                          if (
