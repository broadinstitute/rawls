...
class launchAnalysis extends RawlsSimulation {
	...
	val scn = scenario(s"launchAnalysis_${numUsers}")
		...
		.exec(session => {
			...
			workspaceSubmissionsMap += (attrs.get("workspaceName").get.asInstanceOf[String] -> attrs.get("submissionId").get.asInstanceOf[String])
			session
		})
		...
		exec(session => {
			...
			fileGenerator(new File(filename)) { p =>
				...
				workspaceSubmissionsMap.foreach {
					case (workspaceName, submissionId) => p.println(s"${workspaceName}\t${submissionId}")
				}
