...
import org.broadinstitute.dsde.rawls.model.{AttributeEntityReference, AttributeName, AttributeString, Entity, EntityPointer}
    ...
    "enforce on deleteEntities" in withLocalEntityProviderTestDatabase { dataSource =>
      lockedEntitiesTest(dataSource, a[MySQLTimeoutException]) { localEntityProvider =>
        localEntityProvider.deleteEntities(
          Seq(EntityPointer(entityType = "unitTestType", entityName = "deleteTimeoutTest")),
          testContext
        )
      }
    }
