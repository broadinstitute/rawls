...
class WorkflowSubmissionSpec(_system: ActorSystem)
  ...
  class TestWorkflowSubmission(
    ...
    val methodRepoDAO = new HttpMethodRepoDAO(MethodRepoConfig[Agora.type](mockServer.mockServerBaseUrl, ""),
  ...
  it should "submit a workflow with the right parameters and options for a v1 workspace" in withDefaultTestDatabase {
    ...
    withWorkspaceContext(testData.v1Workspace) { ctx =>
      val (workflowRecs, submissionRec, workspaceRec) =
        getWorkflowSubmissionWorkspaceRecords(testData.submission1, testData.v1Workspace)
      ...
      val petJson =
        Await.result(workflowSubmission.samDAO.getPetServiceAccountKeyForUser(testData.v1Workspace.googleProjectId,
                                                                              testData.userOwner.userEmail
                     ),
                     Duration.Inf
        )
