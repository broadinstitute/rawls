...
class NotificationSpec extends ApiServiceSpec {
  ...
  "Notification" should "be sent for invitation" in withTestDataApiServices { services =>
    ...
    TestKit.awaitCond(services.gpsDAO.messageLog.contains(s"${services.notificationTopic}|${NotificationFormat.write(WorkspaceInvitedNotification("obama@whitehouse.gov", userInfo.userSubjectId, testData.workspace.toWorkspaceName)).compactPrint}"), 10 seconds)
  }
