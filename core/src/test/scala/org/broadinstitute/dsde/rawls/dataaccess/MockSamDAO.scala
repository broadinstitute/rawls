...
import org.broadinstitute.dsde.rawls.model.{RawlsGroupEmail, SubsystemStatus, SyncReportItem, UserInfo, UserStatus}
...
class MockSamDAO extends SamDAO {
  ...
  override def deleteResource(resourceTypeName: SamResourceTypeName, resourceId: String, userInfo: UserInfo): Future[Boolean] = Future.successful(true)
  ...
  override def syncPolicyToGoogle(resourceTypeName: SamResourceTypeName, resourceId: String, policyName: String, userInfo: UserInfo): Future[Map[RawlsGroupEmail, Seq[SyncReportItem]]] = Future.successful(Map.empty)
