...
class BillingApiServiceV2Spec extends ApiServiceSpec with MockitoSugar {
  ...
  it should "return 400 when creating a project with inaccessible to firecloud billing account" in withEmptyDatabaseAndApiServices {
    services =>
      ...
      when(
        services.googleBillingProjectCreator.validateBillingProjectCreationRequest(ArgumentMatchers.eq(request),
  ...
  private def mockPositiveBillingProjectCreation(services: TestApiService,
    ...
    when(
      services.googleBillingProjectCreator
        .validateBillingProjectCreationRequest(any[CreateRawlsV2BillingProjectFullRequest], any[UserInfo])
    )
      .thenReturn(Future.successful())
    when(
      services.googleBillingProjectCreator.postCreationSteps(any[CreateRawlsV2BillingProjectFullRequest], any[UserInfo])
    )
      .thenReturn(Future.successful())
  }
