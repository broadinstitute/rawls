...
class BillingApiServiceV2Spec extends ApiServiceSpec with MockitoSugar {
  ...
  "PATCH /billing/v2/{projectName}/members" should "return 204 when all members exist" in withEmptyDatabaseAndApiServices { services =>
    ...
    Patch(s"/billing/v2/${project.projectName.value}/members",
      BatchProjectAccessUpdate(Set(ProjectAccessUpdate("user1@test.edu", ProjectRoles.Owner), ProjectAccessUpdate("user2@test.edu", ProjectRoles.User)), Set.empty)
    ) ~>
      sealRoute(services.billingRoutesV2) ~>
      check {
  ...
  it should "return 409 when trying to add a member that does not exist and inviteUsersNotFound=false" in withEmptyDatabaseAndApiServices { services =>
    val project = createProject("project")
  ...
  it should "return 204 when trying to add a member that does not exist and inviteUsersNotFound=true" in withEmptyDatabaseAndApiServices { services =>
    val project = createProject("project")
    ...
    Patch(s"/billing/v2/${project.projectName.value}/members?inviteUsersNotFound=true",
      BatchProjectAccessUpdate(Set(ProjectAccessUpdate("user1@test.edu", ProjectRoles.Owner), ProjectAccessUpdate("user2@test.edu", ProjectRoles.User)), Set.empty)
    ) ~>
      sealRoute(services.billingRoutesV2) ~>
      check {
