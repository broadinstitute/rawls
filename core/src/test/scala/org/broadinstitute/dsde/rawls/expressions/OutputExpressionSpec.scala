...
class OutputExpressionSpec extends FlatSpec with Matchers {
  ...
  "OutputExpressions" should "parse" in {
    OutputExpression.build("this.gvcf", testAttr, testEntityType) shouldEqual Success(BoundOutputExpression(ThisEntityTarget, AttributeName("default", "gvcf"), testAttr))
    OutputExpression.build("this.with-dash", testAttr, testEntityType) shouldEqual Success(BoundOutputExpression(ThisEntityTarget, AttributeName("default", "with-dash"), testAttr))
    OutputExpression.build("workspace.gvcf", testAttr, testEntityType) shouldEqual Success(BoundOutputExpression(WorkspaceTarget, AttributeName("default", "gvcf"), testAttr))
    OutputExpression.build("workspace.with-dash", testAttr, testEntityType) shouldEqual Success(BoundOutputExpression(WorkspaceTarget, AttributeName("default", "with-dash"), testAttr))
    OutputExpression.build("workspace.library:cohort", testAttr, testEntityType) shouldEqual Success(BoundOutputExpression(WorkspaceTarget, AttributeName("library", "cohort"), testAttr))
    OutputExpression.build("this.library:cohort", testAttr, testEntityType) shouldEqual Success(BoundOutputExpression(ThisEntityTarget, AttributeName("library", "cohort"), testAttr))

    // does not enforce Attribute Namespace constraints
    OutputExpression.build("this.arbitrary:whatever", testAttr, testEntityType) shouldEqual Success(BoundOutputExpression(ThisEntityTarget, AttributeName("arbitrary", "whatever"), testAttr))
    OutputExpression.build("workspace.arbitrary:whatever", testAttr, testEntityType) shouldEqual Success(BoundOutputExpression(WorkspaceTarget, AttributeName("arbitrary", "whatever"), testAttr))
