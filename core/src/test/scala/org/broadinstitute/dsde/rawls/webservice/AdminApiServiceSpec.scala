...
import org.broadinstitute.dsde.rawls.model.UserAuthJsonSupport.{RawlsGroupMemberListFormat, SyncReportFormat, RawlsBillingProjectTransferFormat}
...
class AdminApiServiceSpec extends ApiServiceSpec {
  ...
  it should "return 204 when unregistering a billing project" in withTestDataApiServices { services =>
    ...
    Delete(s"/admin/project/registration/$projectName", httpJson(RawlsBillingProjectTransfer(project, bucket, userInfo.userEmail.value, userInfo.accessToken.value))) ~>
      sealRoute(services.adminRoutes) ~>
      check {
