    ...
    withWorkspaceContext(testData.workspace) { context1 =>
      withWorkspaceContext(workspace2) { context2 =>
        ...
        assertSameElements(
          ...
          runAndWait(entityQuery.checkAndCopyEntities(context2, context1, "SampleSet", Seq("x2"), false)).entitiesCopied
        )
  ...
  it should "copy entities without a conflict with a cycle" in withDefaultTestDatabase {
    ...
    withWorkspaceContext(testData.workspace) { context1 =>
      withWorkspaceContext(workspace2) { context2 =>
        ...
        assertSameElements(
          allEntities.map(_.toReference),
          runAndWait(entityQuery.checkAndCopyEntities(context2, context1, "test", Seq("a1"), false)).entitiesCopied
        )
  ...
  it should "copy entities with a conflict" in withDefaultTestDatabase {
    ...
    withWorkspaceContext(testData.workspace) { context =>
      ...
      assertResult(Set(EntityHardConflict(testData.sample1.entityType, testData.sample1.name))) {
        runAndWait(
          entityQuery.checkAndCopyEntities(context, context, "Sample", Seq("sample1"), false)
        ).hardConflicts.toSet
      }
