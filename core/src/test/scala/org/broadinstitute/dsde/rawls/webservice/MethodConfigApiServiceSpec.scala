...
class MethodConfigApiServiceSpec extends ApiServiceSpec with TestDriverComponent with ScalaFutures {
  ...
  it should "validate attribute syntax in create method configuration" in withTestDataApiServices { services =>
    ...
    val expectedFailureInputs = Map(
      "goodAndBad.goodAndBadTask.bad_in" -> "Error while parsing the expression. The value you entered is not in the correct format for this data type. For a string, format input as \"value\". For an array, format input as [value1, value2]. For a file, format input as \"gs://\". For a boolean, format input as `true` or `false`. Offending symbol is on line 1 at position 0. Error: mismatched input 'does' expecting {'{', '[', 'workspace.', 'this', 'true', 'false', 'null', STRING, NUMBER}"
    )
    val expectedSuccessOutputs = Seq("goodAndBad.goodAndBadTask.good_out", "empty_out")
    val expectedFailureOutputs = Map(
      "goodAndBad.goodAndBadTask.bad_out" -> "Error while parsing the expression. The value you entered is not in the correct format for this data type. For a string, format input as \"value\". For an array, format input as [value1, value2]. For a file, format input as \"gs://\". For a boolean, format input as `true` or `false`. Offending symbol is on line 1 at position 0. Error: mismatched input 'also' expecting {'{', '[', 'workspace.', 'this', 'true', 'false', 'null', STRING, NUMBER}"
    )
  ...
  it should "validate attribute syntax in create method configuration without data model" in withTestDataApiServices {
    services =>
      ...
      val expectedSuccessInputs = Seq("goodAndBad.goodAndBadTask.good_in")
      val expectedFailureInputs = Map(
        "goodAndBad.goodAndBadTask.bad_in" -> "Error while parsing the expression. The value you entered is not in the correct format for this data type. For a string, format input as \"value\". For an array, format input as [value1, value2]. For a file, format input as \"gs://\". For a boolean, format input as `true` or `false`. Offending symbol is on line 1 at position 0. Error: mismatched input 'blah' expecting {'{', '[', 'workspace.', 'this', 'true', 'false', 'null', STRING, NUMBER}"
      )
      val expectedSuccessOutputs = Seq("goodAndBad.goodAndBadTask.good_out", "empty_out")
      val expectedFailureOutputs = Map(
