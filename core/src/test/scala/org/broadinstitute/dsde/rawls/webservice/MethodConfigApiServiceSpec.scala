...
class MethodConfigApiServiceSpec extends ApiServiceSpec with TestDriverComponent with ScalaFutures {
  ...
  it should "validate attribute syntax in create method configuration" in withTestDataApiServices { services =>
    ...
    val expectedFailureInputs = Map(
      "goodAndBad.goodAndBadTask.bad_in" -> "Error while parsing the expression. The value you entered is not in the correct format for this data type. For a string, format input as \"value\". For an array, format input as {\"value1\", \"value2\"}. For a file, format input as \"gs://\". For a boolean, format input as `true` or `false`."
    )
    val expectedSuccessOutputs = Seq("goodAndBad.goodAndBadTask.good_out", "empty_out")
    val expectedFailureOutputs = Map(
      "goodAndBad.goodAndBadTask.bad_out" -> "Error while parsing the expression. The value you entered is not in the correct format for this data type. For a string, format input as \"value\". For an array, format input as {\"value1\", \"value2\"}. For a file, format input as \"gs://\". For a boolean, format input as `true` or `false`."
    )
