...
//    }
  ...
  private def createTestBillingProject(services: TestApiService, project: RawlsBillingProject, ownerUserInfo: UserInfo, workspaceCreators: Set[UserInfo]) = {
    val policies = Map(
      SamBillingProjectPolicyNames.workspaceCreator -> SamPolicy(workspaceCreators.map(_.userEmail.value), Set.empty, Set(SamProjectRoles.workspaceCreator)),
  ...
  val projectOwnerUserInfo = UserInfo(RawlsUserEmail("project-owner-access"), OAuth2BearerToken("token"), 123, testData.userProjectOwner.userSubjectId)
  val ownerUserInfo = UserInfo(RawlsUserEmail("owner-access"), OAuth2BearerToken("token"), 123, testData.userOwner.userSubjectId)
  val writerUserInfo = UserInfo(RawlsUserEmail("writer-access"), OAuth2BearerToken("token"), 123, testData.userWriter.userSubjectId)
  val readerUserInfo = UserInfo(RawlsUserEmail("reader-access"), OAuth2BearerToken("token"), 123, testData.userReader.userSubjectId)

  val defaultProject = billingProjectFromName("defaultProject")
  val defaultWorkspace = WorkspaceRequest(namespace = defaultProject.projectName.value, name = "defaultWorkspace", Map.empty)

  "WorkspaceApiService" should "return 201 for post to workspaces" in withEmptyWorkspaceApiServices { services =>
    ...
    Post(s"/workspaces", httpJson(defaultWorkspace)) ~> sealRoute(withOwnerAccess.workspaceRoutes) ~>
      check {
  ...
  it should "return 400 for post to workspaces with not ready project" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    createTestBillingProject(withOwnerAccess, defaultProject, projectOwnerUserInfo, Set(ownerUserInfo))

    Seq(CreationStatuses.Creating, CreationStatuses.Error).foreach { projectStatus =>
      runAndWait(rawlsBillingProjectQuery.updateBillingProjects(Seq(defaultProject.copy(status = projectStatus))))
  ...
  it should "return 403 on create workspace with invalid-namespace attributes" in withEmptyWorkspaceApiServices { services =>
    val invalidAttrNamespace = "invalid"

    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    createTestBillingProject(withOwnerAccess, defaultProject, projectOwnerUserInfo, Set(ownerUserInfo))

    val workspace = defaultWorkspace.copy(attributes = Map(AttributeName(invalidAttrNamespace, "attribute") -> AttributeString("foo")))
  ...
  it should "return 403 on create workspace with library-namespace attributes" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    createTestBillingProject(withOwnerAccess, defaultProject, projectOwnerUserInfo, Set(ownerUserInfo))

    val workspace = defaultWorkspace.copy(attributes = Map(AttributeName(AttributeName.libraryNamespace, "attribute") -> AttributeString("foo")))
  ...
  it should "concurrently create workspaces" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    createTestBillingProject(withOwnerAccess, defaultProject, projectOwnerUserInfo, Set(ownerUserInfo))

    def generator(i: Int): ReadAction[Option[Workspace]] = {
      val newWorkspace = WorkspaceRequest(
        namespace = defaultProject.projectName.value,
  ...
  it should "get a workspace" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    createTestBillingProject(withOwnerAccess, defaultProject, projectOwnerUserInfo, Set(ownerUserInfo))
    ...
    Get(defaultWorkspace.toWorkspaceName.path) ~>
      sealRoute(withOwnerAccess.workspaceRoutes) ~>
      check {
  ...
  it should "return 404 getting a non-existent workspace" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    Get(s"/workspaces/not/real") ~>
      sealRoute(withOwnerAccess.workspaceRoutes) ~>
      check {
  ...
  it should "delete a workspace" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    createTestBillingProject(withOwnerAccess, defaultProject, projectOwnerUserInfo, Set(ownerUserInfo))
    ...
    Delete(defaultWorkspace.toWorkspaceName.path) ~>
      sealRoute(withOwnerAccess.workspaceRoutes) ~>
      check {
      ...
      assertResult(None) {
        runAndWait(workspaceQuery.findByName(defaultWorkspace.toWorkspaceName))
      }
  }

//  it should "delete all entities when deleting a workspace" in withTestDataApiServices { services =>
//    val project = billingProjectFromName("foo-bar")
//    createTestBillingProject(services, project, userInfo)
//
//    val newWorkspace = WorkspaceRequest(
//      namespace = project.projectName.value,
//      name = s"newWorkspace",
//      Map.empty
//    )
//
...
//  }
//
  it should "return 400 on update with workspace attributes that specify list as value" in withEmptyWorkspaceApiServices { services =>
    ...
    Patch(defaultWorkspace.toWorkspaceName.path, httpJson(testPayload)) ~>
      sealRoute(withOwnerAccess.workspaceRoutes) ~>
      check {
        assertResult(StatusCodes.BadRequest) {
          status
  ...
  it should "concurrently update workspace attributes" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    createTestBillingProject(withOwnerAccess, defaultProject, projectOwnerUserInfo, Set(ownerUserInfo))
    ...
    def generator(i: Int): ReadAction[Option[Workspace]] = {
      Patch(defaultWorkspace.toWorkspaceName.path, httpJson(Seq(AddUpdateAttribute(AttributeName.withDefaultNS("boo"), AttributeString(s"bang$i")): AttributeUpdateOperation))) ~>
        sealRoute(withOwnerAccess.workspaceRoutes) ~> check {
          assertResult(StatusCodes.OK, responseAs[String]) {
            status
          }
          workspaceQuery.findByName(defaultWorkspace.toWorkspaceName)
...
//  }
  ...
  it should "return 403 on clone workspace when adding invalid-namespace attributes" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    createTestBillingProject(withOwnerAccess, defaultProject, projectOwnerUserInfo, Set(ownerUserInfo))
    ...
    val workspaceCopy = WorkspaceRequest(
      namespace = defaultWorkspace.namespace,
      name = "test_copy",
      Map(AttributeName(invalidAttrNamespace, "attribute") -> AttributeString("foo"))
    )
    Post(s"${defaultWorkspace.toWorkspaceName.path}/clone", httpJson(workspaceCopy)) ~>
      sealRoute(withOwnerAccess.workspaceRoutes) ~>
      check {
        assertResult(StatusCodes.Forbidden) {
          status
...
//  }
  ...
  it should "return 409 Conflict on clone if the destination already exists" in withEmptyWorkspaceApiServices { services =>
    ...
    val workspaceCopy = WorkspaceRequest(namespace = defaultWorkspace.namespace, name = defaultWorkspace.name, Map.empty)
    Post(s"${defaultWorkspace.toWorkspaceName.path}/clone", httpJson(workspaceCopy)) ~>
      sealRoute(withOwnerAccess.workspaceRoutes) ~>
      check {
  ...
  it should "return 404 when requesting an ACL from a non-existent workspace" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
  ...
  it should "return 200 when replacing an ACL for an existing workspace" in withEmptyWorkspaceApiServices { services =>
    val withOwnerAccess = services.withUser(ownerUserInfo.userEmail.value)
    createTestBillingProject(withOwnerAccess, defaultProject, projectOwnerUserInfo, Set(ownerUserInfo))
    ...
    Patch(s"${defaultWorkspace.toWorkspaceName.path}/acl", httpJsonEmpty) ~>
      sealRoute(withOwnerAccess.workspaceRoutes) ~>
      check {
        assertResult(StatusCodes.OK) { status }
      }
  ...
  it should "return 404 when replacing an ACL on a non-existent workspace" in withEmptyWorkspaceApiServices { services =>
    ...
    val project = billingProjectFromName(nonExistent.namespace)
    createTestBillingProject(withOwnerAccess, project, projectOwnerUserInfo, Set(ownerUserInfo))
  ...
  it should "allow an project-owner-access user to get a workspace" in withEmptyWorkspaceApiServices { services =>
    ...
    Get(s"${defaultWorkspace.toWorkspaceName.path}") ~>
  ...
  it should "allow a write-access user to get a workspace" in withEmptyWorkspaceApiServices { services =>
    ...
    Get(s"${defaultWorkspace.toWorkspaceName.path}") ~>
      sealRoute(withWriteAccess.workspaceRoutes) ~>
      check {
        assertResult(StatusCodes.OK) {
          status
        }
  ...
  it should "allow a read-access user to get a workspace" in withEmptyWorkspaceApiServices { services =>
    ...
    Get(s"${defaultWorkspace.toWorkspaceName.path}") ~>
      sealRoute(withReadAccess.workspaceRoutes) ~>
      check {
        assertResult(StatusCodes.OK) {
          status
        }
  ...
  it should "check that an update to a workspace modifies the last modified date" in withEmptyWorkspaceApiServices { services =>
    ...
    val initialDate = Get(s"${defaultWorkspace.toWorkspaceName.path}") ~>
      sealRoute(withOwnerAccess.workspaceRoutes) ~>
      check {
    ...
    Get(s"${defaultWorkspace.toWorkspaceName.path}") ~>
      ...
      check {
        ...
        assertWorkspaceModifiedDate(status, updatedWorkspace)
        assert {
          updatedWorkspace.lastModified.isAfter(initialDate)
        }
      }
  }
