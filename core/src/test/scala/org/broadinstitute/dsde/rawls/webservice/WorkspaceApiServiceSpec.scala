...
class WorkspaceApiServiceSpec extends ApiServiceSpec {
  ...
  class TestWorkspaces() extends TestData {
    ...
    val methodConfig = MethodConfiguration("dsde", "testConfig", "Sample", Map("ready"-> AttributeString("true")), Map("param1"-> AttributeString("this.foo")), Map("out1" -> AttributeString("this.bar"), "out2" -> AttributeString("this.splat")), MethodRepoMethod(workspaceName.namespace, "method-a", 1))
  ...
  it should "create a workspace with a Realm" in withTestDataApiServices { services =>
    ...
    val workspaceWithRealm = WorkspaceRequest(
      namespace = testData.wsName.namespace,
      name = "newWorkspace",
      authorizationDomain = Option(Set(realmGroup)),
  ...
  it should "create a workspace with a multi-group auth domain" in withTestDataApiServices { services =>
    ...
    val workspaceWithRealm = WorkspaceRequest(
      namespace = testData.wsName.namespace,
      name = "newWorkspace2",
      authorizationDomain = Option(Set(realmGroup, realmGroup2, realmGroup3)),
      attributes = Map.empty
    )
  ...
  it should "clone a workspace if the source has a multi-group auth domain and user is in all groups" in withTestDataApiServices { services =>
    ...
    val workspaceCopy = WorkspaceRequest(namespace = workspaceWithRealm.namespace, name = "test_copy", Map.empty, workspaceWithRealm.authorizationDomain)
  ...
  it should "clone a workspace if the user added a group to the source authorization domain" in withTestDataApiServices { services =>
    ...
    val workspaceCopy = WorkspaceRequest(namespace = workspaceWithRealm.namespace, name = "test_copy", Map.empty, Option(Set(realmGroup, realmGroup2, realmGroup3)))
    Post(s"${testData.workspace.path}/clone", httpJson(workspaceCopy)) ~>
      sealRoute(services.workspaceRoutes) ~>
      check {
        assertResult(StatusCodes.Created, response.entity.asString) {
    ...
    Get("/workspaces") ~>
      ...
      check {
        ...
        assertResult(Set(
          ...
          WorkspaceListResponse(WorkspaceAccessLevels.NoAccess, testWorkspaces.workspace3.copy(lastModified = dateTime), WorkspaceSubmissionStats(None, None, 0), Seq.empty, Some(false))
        )) {
  ...
  it should "not allow changing a workspace's Realm if it exists" in withTestDataApiServices { services =>
    ...
    val workspaceWithRealm = WorkspaceRequest(
      namespace = testData.wsName.namespace,
      name = "newWorkspace",
      authorizationDomain = Option(Set(realmGroup1)),
      attributes = Map.empty
    )
    ...
    val workspaceCopy = WorkspaceRequest(namespace = workspaceWithRealm.namespace, name = "test_copy", Map.empty, Option(Set(realmGroup2)))
    Post(s"${workspaceWithRealm.path}/clone", httpJson(workspaceCopy)) ~>
      sealRoute(services.workspaceRoutes) ~>
      check {
  ...
  it should "set the Realm when cloning a workspace with no Realm" in withTestDataApiServices { services =>
    val workspaceCopyNoRealm = WorkspaceRequest(namespace = testData.workspace.namespace, name = "test_copy", Map.empty)
    ...
    val workspaceCopyRealm = WorkspaceRequest(namespace = testData.workspace.namespace, name = "test_copy2", Map.empty, Option(Set(realmGroup)))
  ...
  it should "return 403 when creating a workspace and pointing the realmRef to a regular group" in withTestDataApiServices { services =>
    ...
    val workspaceWithRealm = WorkspaceRequest(
      namespace = testData.wsName.namespace,
      name = "newWorkspace",
      authorizationDomain = Option(Set(ManagedGroupRef(realmGroup.groupName))),
      attributes = Map.empty
    )
  ...
  it should "prevent users not in a realm from accessing workspace" in {
    ...
    val request = WorkspaceRequest(
      ...
      name = "newWorkspace",
      Map.empty,
      Option(Set(ManagedGroupRef(RawlsGroupName(realmName))))
    )
