...
class MockGoogleServicesDAO(groupsPrefix: String,
  ...
  override def isBillingAccountEnabled(billingAccount: RawlsBillingAccountName)(implicit executionContext: ExecutionContext): Future[Boolean] =
    Future.successful(billingAccount == accessibleBillingAccountName)

  override def areServicesEnabled(project: GoogleProject, services: List[String])(implicit executionContext: ExecutionContext): Boolean = true
}
