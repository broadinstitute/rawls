...
class MockGoogleServicesDAO(groupsPrefix: String,
  ...
  def getBucketUsage(googleProject: GoogleProjectId, bucketName: String, maxResults: Option[Long]): Future[BucketUsageResponse] = Future.successful(BucketUsageResponse(42, DateTime.now()))
