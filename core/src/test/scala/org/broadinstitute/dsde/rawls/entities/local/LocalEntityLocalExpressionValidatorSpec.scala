...
class LocalEntityLocalExpressionValidatorSpec extends AnyFlatSpec with TestDriverComponent with LocalExpressionFixture with RawlsTestUtils with ScalaFutures  {
  ...
  it should "parse method config expressions" in {
    ...
    val actualParseableWithNoRoot = expressionValidator.validateMCExpressionsInternal(toExpressionMap(parseableInputExpressionsWithNoRoot), toExpressionMap(parseableOutputExpressionsWithNoRoot), None)
    ...
    val actualUnparseableWithNoRoot = expressionValidator.validateMCExpressionsInternal(toExpressionMap(unparseableInputExpressionsWithNoRoot), toExpressionMap(unparseableOutputExpressionsWithNoRoot), None)
  ...
  it should "validate output expressions" in {
    assert(expressionValidator.validateOutputExpr(defaultRootEntity)(expression = "this.attribute").isSuccess, "this.attribute should parse correctly" )
  ...
  it should "validate library output expressions" in {
    assert(expressionValidator.validateOutputExpr(defaultRootEntity)(expression = "this.library:attribute").isSuccess, "this.library:attribute should parse correctly" )
