...
class WorkspaceServiceSpec extends FlatSpec with ScalatestRouteTest with Matchers with TestDriverComponent with RawlsTestUtils {
  ...
  it should "return non-existent users during patch ACLs" in withTestDataServices { services =>
    ...
    val vComplete = Await.result(services.workspaceService.updateACL(testData.workspace.toWorkspaceName, aclUpdates, false), Duration.Inf)
      .asInstanceOf[RequestComplete[(StatusCode, WorkspaceACLUpdateResponseList)]]
    val responseFromUpdate = WorkspaceACLUpdateResponseList(Seq.empty, Seq.empty, Seq(WorkspaceACLUpdate("obama@whitehouse.gov", WorkspaceAccessLevels.Owner, None)))
    ...
    val vComplete2 = Await.result(services.workspaceService.updateACL(testData.workspace.toWorkspaceName, aclUpdates2, true), Duration.Inf)
      .asInstanceOf[RequestComplete[(StatusCode, WorkspaceACLUpdateResponseList)]]
    val responseFromUpdate2 = WorkspaceACLUpdateResponseList(Seq.empty, Seq(WorkspaceACLUpdate("obama@whitehouse.gov", WorkspaceAccessLevels.Owner, None)), Seq.empty)
    ...
    val vComplete3 = Await.result(services.workspaceService.updateACL(testData.workspace.toWorkspaceName, aclUpdates3, true), Duration.Inf)
      .asInstanceOf[RequestComplete[(StatusCode, WorkspaceACLUpdateResponseList)]]
    val responseFromUpdate3 = WorkspaceACLUpdateResponseList(Seq.empty, Seq(WorkspaceACLUpdate("obama@whitehouse.gov", WorkspaceAccessLevels.Read, None)), Seq.empty)
  ...
  it should "invite a user to a workspace" in withTestDataServices { services =>
    ...
    assert(vComplete.response._2.invitesUpdated.contains(WorkspaceACLUpdate("obama@whitehouse.gov", WorkspaceAccessLevels.Owner, None)))
  ...
  it should "delete a workspace with succeeded submission" in withTestDataServices { services =>
    ...
    assertResult(Set((testData.userOwner.userEmail.value, WorkspaceAccessLevels.Owner, true), (testData.userOwner.userEmail.value, WorkspaceAccessLevels.ProjectOwner, true), ("project-owner-access", WorkspaceAccessLevels.ProjectOwner, true), (testData.userReader.userEmail.value,WorkspaceAccessLevels.Read, false), (testData.userWriter.userEmail.value,WorkspaceAccessLevels.Write, false))) {
      runAndWait(workspaceQuery.listEmailsAndAccessLevel(SlickWorkspaceContext(testData.workspaceSuccessfulSubmission)))._1.toSet
    }
  ...
  it should "delete a workspace with failed submission" in withTestDataServices { services =>
    ...
    //Check if access levels on workspace exist
    assertResult(Set((testData.userOwner.userEmail.value, WorkspaceAccessLevels.Owner, true), (testData.userOwner.userEmail.value, WorkspaceAccessLevels.ProjectOwner, true), ("project-owner-access", WorkspaceAccessLevels.ProjectOwner, true), (testData.userReader.userEmail.value,WorkspaceAccessLevels.Read, false), (testData.userWriter.userEmail.value,WorkspaceAccessLevels.Write, false))) {
      runAndWait(workspaceQuery.listEmailsAndAccessLevel(SlickWorkspaceContext(testData.workspaceFailedSubmission)))._1.toSet
    }
  ...
  it should "delete a workspace with submitted submission" in withTestDataServices { services =>
    ...
    //Check if access levels on workspace exist
    assertResult(Set((testData.userOwner.userEmail.value, WorkspaceAccessLevels.Owner, true), (testData.userOwner.userEmail.value, WorkspaceAccessLevels.ProjectOwner, true), ("project-owner-access", WorkspaceAccessLevels.ProjectOwner, true), (testData.userReader.userEmail.value,WorkspaceAccessLevels.Read, false), (testData.userWriter.userEmail.value,WorkspaceAccessLevels.Write, false))) {
      runAndWait(workspaceQuery.listEmailsAndAccessLevel(SlickWorkspaceContext(testData.workspaceSubmittedSubmission)))._1.toSet
    }
  ...
  it should "delete a workspace with mixed submissions" in withTestDataServices { services =>
    ...
    //Check if access levels on workspace exist
    assertResult(Set((testData.userOwner.userEmail.value, WorkspaceAccessLevels.Owner, true), (testData.userOwner.userEmail.value, WorkspaceAccessLevels.ProjectOwner, true), ("project-owner-access", WorkspaceAccessLevels.ProjectOwner, true), (testData.userReader.userEmail.value,WorkspaceAccessLevels.Read, false), (testData.userWriter.userEmail.value,WorkspaceAccessLevels.Write, false))) {
      runAndWait(workspaceQuery.listEmailsAndAccessLevel(SlickWorkspaceContext(testData.workspaceMixedSubmissions)))._1.toSet
    }
