...
class WorkspaceServiceSpec
  ...
  class TestApiService(dataSource: SlickDataSource, val user: RawlsUser)(implicit
    ...
    override val multiCloudWorkspaceServiceConstructor: UserInfo => MultiCloudWorkspaceService =
    ...
    val workspaceServiceConstructor = WorkspaceService.constructor(
      slickDataSource,
      new HttpMethodRepoDAO(MethodRepoConfig[Agora.type](mockServer.mockServerBaseUrl, ""),
  ...
  it should "return an error if an MC workspace is not present in workspace manager" in withTestDataServices {
    services =>
      ...
      when(services.workspaceManagerDAO.getWorkspace(any[UUID], any[OAuth2BearerToken])).thenAnswer(_ =>
  ...
  "listWorkspaces" should "list the correct cloud platform for Azure and Google workspaces" in withTestDataServices {
    services =>
      ...
      // mock external calls
      when(service.workspaceManagerDAO.getWorkspace(azureWorkspace.workspaceIdAsUUID, services.userInfo1.accessToken))
        .thenReturn(new WorkspaceDescription().azureContext(new AzureContext()))
      when(service.workspaceManagerDAO.getWorkspace(googleWorkspace.workspaceIdAsUUID, services.userInfo1.accessToken))
        .thenReturn(
      ...
      val result = Await
        .result(service.listWorkspaces(WorkspaceFieldSpecs(), null), Duration.Inf)
  ...
  "listWorkspaces" should "return an error if an MC workspace does not have a cloud context" in withTestDataServices {
    services =>
      ...
      val err = intercept[RawlsException] {
        Await.result(service.listWorkspaces(WorkspaceFieldSpecs(), null), Duration.Inf)
      }
  }
