...
class ExpressionEvaluationSupportSpec extends AnyFlatSpec with Matchers with ExpressionEvaluationSupport {
  ...
  val stringConversions = Map(
    ...
    AttributeNumber(Double.MaxValue) -> AttributeString("1.7976931348623157E+308"), // should we disable scientific notation??
    AttributeNumber(Double.MinValue) -> AttributeString("-1.7976931348623157E+308"), // should we disable scientific notation??
    AttributeNumber(Double.MinPositiveValue) -> AttributeString("4.9E-324"), // should we disable scientific notation??
