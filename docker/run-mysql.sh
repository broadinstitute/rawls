...
start() {
    ...
    docker run --name $CONTAINER -e MYSQL_ROOT_HOST='%' -e MYSQL_ROOT_PASSWORD=rawls-test -e MYSQL_USER=rawls-test -e MYSQL_PASSWORD=rawls-test \
                -e MYSQL_DATABASE=testdb -d -p 3310:3306 $MYSQL_IMAGE:$MYSQL_VERSION \
                --character-set-server=utf8mb4 --log_bin_trust_function_creators=ON
