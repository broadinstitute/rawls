...
class HttpGoogleAccessContextManagerDAO(clientEmail: String, pemFile: String, appName: String, serviceProject: String, override val workbenchMetricBaseName: String)
  ...
  def overwriteProjectsInServicePerimeter(servicePerimeterName: ServicePerimeterName, googleProjectNumbers: Set[String]): Future[Operation] = {
    ...
    retryWhen500orGoogleError(() =>
    {
      ...
      val fullyQualifiedProjects: List[String] = googleProjectNumbers.map("projects/" + _).toList
