...
class WorkspaceService(workspaceDAO: WorkspaceDAO) extends Actor {
  override def receive = {
    ...
    case CloneWorkspace(sourceNamespace, sourceWorkspace, destNamespace, destWorkspace) => cloneWorkspace(sourceNamespace, sourceWorkspace, destNamespace, destWorkspace)
  }
...
//    }
  ...
  def cloneWorkspace(sourceNamespace:String, sourceWorkspace:String, destNamespace:String, destWorkspace:String): Unit = {
    ...
    (originalWorkspace, copyWorkspace) match {
      case ( Some(ws), None ) => {
        ...
        context.parent ! PerRequest.RequestComplete((StatusCodes.Created, newWorkspace))
      }
