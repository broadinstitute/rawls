...
import org.broadinstitute.dsde.rawls.model.{RawlsBillingProject, RawlsBillingProjectName, CreationStatuses}
...
trait CreatingBillingProjectMonitor {
  ...
  def checkCreatingProjects()(implicit executionContext: ExecutionContext): Future[CheckDone] = {
    for {
      ...
      readyProjects <- setupProjects(creatingProjects)
  ...
  def setupProjects(projects: Seq[RawlsBillingProject])(implicit executionContext: ExecutionContext): Future[Seq[RawlsBillingProject]] = {
    Future.traverse(projects) { project =>
      gcsDAO.setupProject(project, projectTemplate).map((project, _))
    } map {
      _.collect {
