  ...
  private def handleStatusChange(workflow: Workflow, workflowOutputsOption: Option[Map[String, Attribute]]): Future[Unit] = {
    val savedWorkflowFuture = datasource.inTransaction { dataAccess =>
      withWorkspaceContext(workspaceName, dataAccess) { workspaceContext =>
        ...
        val workflowToSave = saveOutputs.getOrElse(DBIO.successful(Success(Unit))).map {
        ...
        workflowToSave.flatMap(wf =>
            dataAccess.workflowQuery.update(workspaceContext, UUID.fromString(submissionId), wf.copy(statusLastChangedDate = DateTime.now)))
      }
    }
      ...
      withWorkspaceContext(workspaceName, dataAccess) { workspaceContext =>
        dataAccess.submissionQuery.get(workspaceContext, submissionId) flatMap {
          ...
          case Some(refreshedSubmission) if (refreshedSubmission.workflows.forall(workflow => workflow.status.isDone)) =>
            val updateAction = refreshedSubmission.status match {
              ...
              case unexpectedState =>
                DBIO.failed(new RawlsException(s"submission $submissionId is in an unexpected state: $unexpectedState"))
            }
            updateAction.map {_ => true }
          case _ => DBIO.successful(false)
        }
      }
    } map { shouldStop => if(shouldStop) stop(self) } // this should run AFTER the transaction commits
  }
