...
trait SubmissionMonitor extends FutureSupport with LazyLogging {
  ...
  def listWorkflowEntitiesById(workflowsWithOutputs: Seq[(WorkflowRecord, ExecutionServiceOutputs)], dataAccess: DataAccess): ReadAction[scala.collection.Map[Long, Entity]] = {
