...
trait WorkspaceComponent {
  ...
  object workspaceQuery extends TableQuery(new WorkspaceTable(_)) {
    ...
    def listSubmissionSummaryStats(workspaceIds: Seq[UUID]): ReadAction[Map[UUID, WorkspaceSubmissionStats]] = {
      ...
      implicit val queryToStatRow = GetResult(r => StatRow(r.<<, r.<<, r.<<, r.<<))

      // by using a literal string inside the where clause, we are technically vulnerable to SQL injection.
      // however, the UUIDs sent in to the literal are generated by the database; the user has no
      // direct access to this argument
      val inUUIDs = workspaceIds.map{u => "X'" + u.toString.replace("-","") + "'"}.mkString(",")

      // we abuse UNION here to optimize database interaction. We make one query to return both the
      // count-of-running-submissions and the last-succeeded/last-failed workflows.
      // the 'cnt' column will be -1 for workflows, and a value for submissions.
      import driver.quoteIdentifier
      val sql:ReadAction[Seq[StatRow]] =
        sql"""select w.#${quoteIdentifier("id")} as workspaceId,
                count(s.#${quoteIdentifier("ID")}) as cnt,
                s.#${quoteIdentifier("DATE_SUBMITTED")} as dt,
                s.#${quoteIdentifier("STATUS")} as status
              from SUBMISSION s
                left join WORKSPACE w on w.#${quoteIdentifier("id")} = s.#${quoteIdentifier("WORKSPACE_ID")}
              where w.#${quoteIdentifier("id")} in (#$inUUIDs)
                and s.#${quoteIdentifier("STATUS")} = ${SubmissionStatuses.Submitted.toString}
              group by w.#${quoteIdentifier("id")}
            UNION
              select w.#${quoteIdentifier("id")} as workspaceId,
                -1 as cnt,
                max(wf.#${quoteIdentifier("STATUS_LAST_CHANGED")}) as dt,
                wf.#${quoteIdentifier("STATUS")} as status
              from SUBMISSION s
                left join WORKSPACE w on w.#${quoteIdentifier("id")} = s.#${quoteIdentifier("WORKSPACE_ID")}
                left join WORKFLOW wf on wf.#${quoteIdentifier("SUBMISSION_ID")} = s.#${quoteIdentifier("ID")}
              where w.#${quoteIdentifier("id")} in (#$inUUIDs)
                and (wf.#${quoteIdentifier("STATUS")} = ${WorkflowStatuses.Succeeded.toString}
                     or wf.#${quoteIdentifier("STATUS")} = ${WorkflowStatuses.Failed.toString})
              group by w.#${quoteIdentifier("id")}, wf.#${quoteIdentifier("STATUS")};""".as[StatRow]

      // transform from individual rows to Map[UUID, WorkspaceSubmissionStats]
      sql.map {allRows:Seq[StatRow] =>
        val foundWorkspaces:Map[UUID, WorkspaceSubmissionStats] = allRows
          .groupBy(_.workspaceId)
          .map {wsStats:(UUID, Seq[StatRow]) =>
            val wsRows:Seq[StatRow] = wsStats._2
            val runningSubmissions = wsRows.find(_.cnt > 0).map{_.cnt}.getOrElse(0)
            val lastSuccessDate = wsRows.find(wf => -1==wf.cnt && wf.status==WorkflowStatuses.Succeeded.toString).map{_.dt}
            val lastFailedDate = wsRows.find(wf => -1==wf.cnt && wf.status==WorkflowStatuses.Failed.toString).map{_.dt}
            (wsStats._1, WorkspaceSubmissionStats(lastSuccessDate, lastFailedDate, runningSubmissions))
        }

        // the sql query may not have returned information for every requested workspace.
        // create empty/default WorkspaceSubmissionStats for the remaining ones
        workspaceIds.map{uuid => (uuid, foundWorkspaces.getOrElse(uuid, WorkspaceSubmissionStats(None, None, 0))) }.toMap
      }
    }
