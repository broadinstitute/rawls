...
abstract class GoogleServicesDAO(groupsPrefix: String) extends ErrorReportable {
  ...
  def toBillingProjectGroupName(billingProjectName: RawlsBillingProjectName, role: ProjectRoles.ProjectRole): String
  ...
  def createProject(projectName: RawlsBillingProjectName, creator: RawlsUser, billingAccount: RawlsBillingAccount, projectTemplate: ProjectTemplate): Future[Unit]
