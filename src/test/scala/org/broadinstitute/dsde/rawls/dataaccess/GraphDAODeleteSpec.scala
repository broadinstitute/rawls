...
class GraphDAODeleteSpec extends FreeSpec with Matchers with OrientDbTestFixture {
  ...
  val workspace = Workspace(wsName.namespace, wsName.name, "aWorkspaceId", "aBucket", DateTime.now, "testUser", Map.empty[String, Attribute] )
  val wsWithAttributeVals = Workspace(wsName.namespace, wsName.name, "aWorkspaceId", "aBucket", DateTime.now, "testUser",
    Map(
