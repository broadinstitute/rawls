...
class MethodConfigResolverSpec extends WordSpecLike with Matchers with TestDriverComponent {
  ...
  val workspace = new Workspace("workspaces", "test_workspace", None, UUID.randomUUID().toString(), "aBucket", DateTime.now, DateTime.now, "testUser", Map.empty, Map.empty, Map.empty)
