...
class SubmissionMonitorSpec(_system: ActorSystem) extends TestKit(_system) with FlatSpecLike with Matchers with OrientDbTestFixture {
  ...
  "SubmissionMonitor" should "mark unknown workflows" in withDefaultTestDatabase { dataSource =>
    ...
    assertResult(true) { dataSource inTransaction { txn =>
      submissionDAO.get(testData.submission.workspaceNamespace, testData.submission.workspaceName, testData.submission.id, txn).get.workflow.forall(_.status == WorkflowStatuses.Unknown)
    }}
  }
  ...
  it should "transition to running then completed then terminate" in withDefaultTestDatabase { dataSource =>
    ...
    testData.submission.workflow.foreach { workflow =>
    ...
    awaitCond({
      dataSource inTransaction { txn =>
        submissionDAO.get(testData.submission.workspaceNamespace, testData.submission.workspaceName, testData.submission.id, txn).get.workflow.forall(_.status == WorkflowStatuses.Running)
      }
    }, 10 seconds)
    ...
    assertResult(true) { dataSource inTransaction { txn =>
      submissionDAO.get(testData.submission.workspaceNamespace, testData.submission.workspaceName, testData.submission.id, txn).get.workflow.forall(_.status == WorkflowStatuses.Succeeded)
    }}
  }
  ...
  it should "terminate when all workflows are done" in withDefaultTestDatabase { dataSource =>
    ...
    assertResult(WorkflowStatuses.terminalStatuses.size) {
      testData.submission.workflow.size
    }

    testData.submission.workflow.zip(WorkflowStatuses.terminalStatuses).foreach { case (workflow, status) =>
