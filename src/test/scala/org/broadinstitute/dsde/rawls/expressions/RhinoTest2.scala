 ...
 */
class ExpressionParserTest extends FunSuite with OrientDbTestFixture {
  ...
  test("rhino in the orient III") {
    initializeTestGraph
    try {
      println("get the entity type from all samples from all sample sets from the root sampleSets")
      val value = new RNExpressionEvaluator(graph).executeJS("function evaluate(value) {return <root.sampleSet.samples._entityType>}")("sampleSets")
...
class RNExpressionEvaluator(graph:Graph) extends JavaTokenParsers {
