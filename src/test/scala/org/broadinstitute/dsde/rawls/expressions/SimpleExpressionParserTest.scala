  ...
  test("simple attribute expression") {
    withTestWorkspace { (workspaceContext, txn) =>
      ...
      assertResult(Success(ArrayBuffer(AttributeString("normal")))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "Sample", "sample1", "this.type")
      }

      assertResult(Success(ArrayBuffer(AttributeString("normal"), AttributeString("tumor"), AttributeString("tumor")))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "SampleSet", "sset1", "this.samples.type")
      }

      assertResult(Success(ArrayBuffer())) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "SampleSet", "sset_empty", "this.samples")
      }

      assertResult(Success(ArrayBuffer(AttributeString("a"), AttributeBoolean(true)))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "Sample", "sample1", "this.thingies")
      }
    }
  }
  ...
  test("reserved attribute expression") {
    withTestWorkspace { (workspaceContext, txn) =>
      ...
      assertResult(Success(ArrayBuffer(AttributeString("sample1")))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "Sample", "sample1", "this.name")
      }

      assertResult(Success(ArrayBuffer(AttributeString("Sample"), AttributeString("Sample"), AttributeString("Sample")))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "SampleSet", "sset1", "this.samples.entityType")
      }

      assertResult(Success(ArrayBuffer(AttributeString(workspaceContext.workspace.name)))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "SampleSet", "sset1", "workspace.name")
      }

      assertResult(Success(ArrayBuffer(AttributeNull))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "SampleSet", "sset1", "workspace.entityType")
      }
    }
  }
  ...
  test("workspace attribute expression") {
    withTestWorkspace { (workspaceContext, txn) =>
      ...
      assertResult(Success(ArrayBuffer(testData.wsAttrs.get("string").get))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "dummy text", "dummy text", "workspace.string")
      }

      assertResult(Success(ArrayBuffer(testData.wsAttrs.get("number").get))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "dummy text", "dummy text", "workspace.number")
      }
      ...
      assertResult(Success(ArrayBuffer(testData.sample1.attributes.get("type").get))) {
        ...
        evaluator.evalFinalAttribute(workspaceContext, txn, "dummy text", "dummy text", "workspace.sample1ref.type")
      }

      assertResult(Success(ArrayBuffer())) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "dummy text", "dummy text", "workspace.empty")
      }

      assertResult(Success(ArrayBuffer(AttributeString("another string"), AttributeBoolean(true)))) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "dummy text", "dummy text", "workspace.values")
      }

      assertResult(Success(ArrayBuffer())) {
        evaluator.evalFinalAttribute(workspaceContext, txn, "dummy text", "dummy text", "workspace.this_attribute_is_not_present")
      }

      assert {
        evaluator.evalFinalAttribute(workspaceContext, txn, "dummy text", "dummy text", "workspace").isFailure
      }
      ...
      assert {
        evaluator.evalFinalAttribute(workspaceContext, txn, "dummy text", "dummy text", "workspace.missing.also_missing").isFailure
      }
      ...
      assert {
        ...
        evaluator.evalFinalAttribute(workspaceContext, txn, "dummy text", "dummy text", "workspace.sample1ref.").isFailure
      }
    }
  }
  ...
  test("simple entity expression") {
    withTestWorkspace { (workspaceContext, txn) =>
      ...
      assertResult(Success(ArrayBuffer(testData.sample2))) {
        evaluator.evalFinalEntity(workspaceContext, txn, "Pair", "pair1", "this.case")
      }

      assertResult(Success(ArrayBuffer(testData.sample1, testData.sample2, testData.sample3))) {
        evaluator.evalFinalEntity(workspaceContext, txn, "Individual", "indiv1", "this.sset.samples")
      }

      assertResult(Success(ArrayBuffer(testData.pair1))) {
        evaluator.evalFinalEntity(workspaceContext, txn, "Pair", "pair1", "this")
      }
    }
  }
  ...
  test("workspace entity expression") {
    withTestWorkspace { (workspaceContext, txn) =>
      ...
      assertResult(Success(ArrayBuffer(testData.sample1))) {
        ...
        evaluator.evalFinalEntity(workspaceContext, txn, "dummy text", "dummy text", "workspace.sample1ref")
      }
      ...
      assertResult(Success(ArrayBuffer(testData.sample2))) {
        ...
        evaluator.evalFinalEntity(workspaceContext, txn, "dummy text", "dummy text", "workspace.samplerefs")
      }
    }
  }
}
