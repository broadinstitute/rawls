...
class ExpressionParserTest extends FunSuite with OrientDbTestFixture {
  ...
  test("rhino in the orient III") {
    try {
      ...
      val value = new ExpressionEvaluator(graph).executeJS("function evaluate(value) {return <root.sampleSet.samples._entityType>}")("sampleSets")
...
class ExpressionEvaluator(graph:Graph) extends JavaTokenParsers {
