 ...
 */
class RhinoTest extends FunSuite with OrientDbTestFixture {
  ...
  test("rhino") {
    ...
    try {
      ...
      val wrapped = Context.javaToJS(new Foo(), scope);
      ScriptableObject.putProperty(scope, "foo", wrapped);

      // Now we can evaluate a script. Let's create a new object
      // using the object literal notation.
//      val result = cx.evaluateString(scope, "function evaluate(thiz) { return thiz + \"bar!\" + foo.ret() }",
      val result = cx.evaluateString(scope, "function evaluate(thiz) { return thiz + \"bar!\" + foo.ret() }",
        "MySource", 1, null);

      // TODO:
      val eval = scope.get("evaluate", scope).asInstanceOf[Function];
      val that = cx.newObject(scope);
      println(eval.call(cx, scope, that, List("Foo").toArray))
    } finally {
      Context.exit();
    }
  }

  test("rhino in the orient") {
    val cx:Context = Context.enter();
    try {
      // Set version to JavaScript1.2 so that we get object-literal style
      // printing instead of "[object Object]"
      ...
      val value = new QueryExecutor2(scope, graph).executeJS("function evaluate(value) {return <root.sampleSet.samples._entityType>}")("sampleSets")
      println("results: " + value)
      println()

      val result = cx.evaluateString(scope, value,
        "MySource", 1, null);
...
/* Disabled because a) doesn't work b) isn't in line with the ExpressionParsing class
test("rhino in the orient II") {
  ...
  try {
    println("get the entity type from all samples from all sample sets from the root sampleSets")
    val value = new QueryExecutor3(engine, graph).executeJS("function evaluate(value) {return <root.sampleSet.samples._entityType>}")("sampleSets")
    println("results: " + value)
    println()

    val code = engine.eval(value)

      val engineInv:Invocable = engine.asInstanceOf[Invocable];
//        val func = engine.get("evaluate");
      val result = engineInv.invokeFunction("evaluate", "foo");
...
class QueryExecutor2(scope:Scriptable, graph:Graph) extends JavaTokenParsers {
...
def executeJS(expression:String)(rootName:String) = parseAll(jsExpression, expression) match {
  case Success(result, _) => {
    result
  }
  case NoSuccess(msg, next) => {
...
private def addVariableToScope(o:Object, variableName:String) = {
  val wrapped = Context.javaToJS(o, scope);
  ScriptableObject.putProperty(scope, variableName, wrapped);
}
private val rand = new Random()
//  private def jsExpression:Parser[String] = "function evaluate(value) {return " ~ jsPath ~ "}" ^^ {
//    case first ~ path ~ last => first + path + last
//  }
private def jsExpression:Parser[String] = (
    jsCode ~ jsPath ~ jsExpression ^^ {case code ~ path ~ exp => code + path + exp.mkString("")} |
    jsPath |
    jsCode
  )
...
//      first + path + second + rest
//    }
//  }
private def jsPath:Parser[String] = "<" ~> path <~ ">" ^^ {
  case path => {
    val value = applySteps("sampleSets", path)
    val variable = "val_" + Math.abs(rand.nextInt())
    // TODO: need to be able to send List<Number|String|Double|
    addVariableToScope(value.head, variable)
    variable
  }
}
...
class QueryExecutor3(engine:ScriptEngine, graph:Graph) extends JavaTokenParsers {
...
private def jsPath:Parser[String] = "<" ~> path <~ ">" ^^ {
  case path => {
    ...
    val variable = "val_" + Math.abs(rand.nextInt())
    // TODO: need to be able to send List<Number|String|Double|
    addVariableToScope(value, variable)
    variable
  }
}
private def pathList:Parser[List[Seq[String]]] = rep(path <~ "," | path) ^^ {case paths  => paths.map(path => applySteps("sampleSets", path))}
