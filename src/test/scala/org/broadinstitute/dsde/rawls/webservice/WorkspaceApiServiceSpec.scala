...
class WorkspaceApiServiceSpec extends ApiServiceSpec {
  ...
  class TestWorkspaces() extends TestData {
    ...
    val methodConfig = MethodConfiguration("dsde", "testConfig", "Sample", Map("ready"-> AttributeString("true")), Map("param1"-> AttributeString("foo")), Map("out1" -> AttributeString("bar"), "out2" -> AttributeString("splat")), MethodRepoMethod(workspaceName.namespace, "method-a", 1), false)
  ...
  it should "return 403 creating workspace in billing project with no access" in withTestDataApiServices { services =>
    runAndWait(rawlsBillingProjectQuery.save(RawlsBillingProject(RawlsBillingProjectName("foobar"), Set.empty, Set.empty, "mockBucketUrl", CreationStatuses.Ready)))
    val newWorkspace = WorkspaceRequest(
      namespace = "foobar",
      name = "newWorkspace",
      None,
