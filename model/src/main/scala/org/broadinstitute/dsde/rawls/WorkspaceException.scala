...
abstract sealed class WorkspaceException(statusCode: StatusCode, message: String)(implicit source: ErrorReportSource)
...
object WorkspaceAccessDeniedException {
  def apply(workspaceName: WorkspaceName)(implicit source: ErrorReportSource): WorkspaceAccessDeniedException = WorkspaceAccessDeniedException(workspaceName.toString)
}

case class NoSuchWorkspaceException(workspace: String)(implicit source: ErrorReportSource)
  extends WorkspaceException(StatusCodes.NotFound, s"workspace $workspace does not exist or you do not have permission to use it")

object NoSuchWorkspaceException {
  def apply(workspaceName: WorkspaceName)(implicit source: ErrorReportSource): NoSuchWorkspaceException = NoSuchWorkspaceException(workspaceName.toString)
}
...
object LockedWorkspaceException {
  def apply(workspaceName: WorkspaceName)(implicit source: ErrorReportSource): LockedWorkspaceException = LockedWorkspaceException(workspaceName.toString)
}
