...
object Publishing {
  ...
  val publishSettings: Seq[Setting[_]] =
    ...
    Seq(publishTo := Option(artifactoryResolver(false)), credentials += artifactoryCredentials, crossScalaVersions := List("2.12.15", "2.13.2"), publishConfiguration := publishConfiguration.value.withOverwrite(true))
